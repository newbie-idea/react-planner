...
export default function ItemAttributesEditor({element, onUpdate, attributeFormData, state, ...rest}, {translator}) {
  ...
  //New variables to correct scale.
  let trueX = renderedX / 2 * 10;
  let trueY = renderedY / 2 * 10;
    ...
    <table style={tableStyle}>
      <tbody>
        ...
        <tr>
          ...
          <td>
            <FormNumberInput
              value={trueX} //Use new variable to correct scale.
              onChange={event => onUpdate('x', (event.target.value / 10 * 2))} //Double value to correct scale.
              style={inputStyle}
              state={state}
              precision={2}
              {...rest}
        ...
        <tr>
          ...
          <td>
            <FormNumberInput
              value={trueY} //Use new variable to correct scale.
              onChange={event => onUpdate('y', (event.target.value / 10 * 2))} //Double value to correct scale.
              style={inputStyle}
              state={state}
              precision={2}
              {...rest}
