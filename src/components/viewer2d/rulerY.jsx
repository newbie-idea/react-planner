...
export default class RulerY extends Component {
  ...
  render() {
	...
	//Changed "x * 100" to "x * 500" to correct scale.
    if (elementH <= 200) {
      for (let x = 1; x <= this.props.positiveUnitsNumber; x++) {
        positiveDomElements.push(
          <div key={x} style={{ ...elementStyle, gridColumn: 1, gridRow: x }}>
            {elementH > 30 ? ((this.props.positiveUnitsNumber - x) * 500) : ''}
          </div>
        );
      }
    }
      ...
      for (let x = 1; x <= this.props.positiveUnitsNumber; x++) {
        let val = (this.props.positiveUnitsNumber - x) * 500;
        positiveDomElements.push(
          <div key={x} style={{ ...elementStyle, gridColumn: 1, gridRow: x }}>
            <div style={insideElementsStyle}>{val + (4 * 100)}</div>
            <div style={insideElementsStyle}>{val + (3 * 100)}</div>
            <div style={insideElementsStyle}>{val + (2 * 100)}</div>
            <div style={insideElementsStyle}>{val + (1 * 100)}</div>
