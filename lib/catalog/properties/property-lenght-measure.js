...
function PropertyLengthMeasure(_ref, _ref2) {
  ...
  var _unit = value.get('_unit') || _constants.UNIT_MILLIMETER;
  ...
  var update = function update(lengthInput, unitInput) {
    ...
    var merged = value.merge({
      length: unitInput !== _constants.UNIT_MILLIMETER ? (0, _convertUnits2.default)(newLength).from(unitInput).to(_constants.UNIT_MILLIMETER) : newLength,
      _length: lengthInput,
      _unit: unitInput
    });
