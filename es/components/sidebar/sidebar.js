...
import PanelRefresh from './panel-refresh';
import PanelWorktops from './panel-worktops';
import PanelColour from './panel-colour';
...
export default function Sidebar(_ref) {
  ...
  { index: 3, condition: true, dom: React.createElement(PanelRefresh, { state: state }) }, { index: 4, condition: true, dom: React.createElement(PanelLayers, { state: state }) }, { index: 5, condition: true, dom: React.createElement(PanelLayerElements, { mode: state.mode, layers: state.scene.layers, selectedLayer: state.scene.selectedLayer }) }, { index: 6, condition: true, dom: React.createElement(PanelGroups, { mode: state.mode, groups: state.scene.groups, layers: state.scene.layers }) }, { index: 7, condition: !multiselected, dom: React.createElement(PanelElementEditor, { state: state }) },
  //{ index: 8, condition: multiselected, dom: <PanelMultiElementsEditor state={state} /> },
  { index: 9, condition: !!selectedGroup, dom: React.createElement(PanelGroupEditor, { state: state, groupID: selectedGroup ? selectedGroup[0] : null }) }];
