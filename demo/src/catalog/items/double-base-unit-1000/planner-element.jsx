...
import {sceneValues, dbu1000GableArrays} from '../../utils/mtl-exports';

let globalTexture;
let worktopTexture;
let dbu1000GableLeftValue;
let dbu1000GableRightValue;
let kickboardValue;
let hingeNumber;
...
let cachedJSONDoubleBaseUnit1000 = null;

export default {
  name: 'Double Base Unit 1000',
  prototype: 'items',

  info: {
    title: 'DBU 1000',
    tag: ['double', 'base'],
    description: 'Double Base Unit 1000',
    image: require('../../images/DBU-1000.png')
  },
  ...
  render2D: function (element, layer, scene) {
    ...
    dbu1000GableLeftValue = element.properties.get('gableLeft');
    dbu1000GableRightValue = element.properties.get('gableRight');
    dbu1000GableArrays(dbu1000GableLeftValue, dbu1000GableRightValue);
    ...
    if (true == dbu1000GableLeftValue) {
	    lineStyleB = { stroke: '#000', strokeWidth: '1px' };
    }
    
    if (true == dbu1000GableRightValue) {
  ...
  render3D: function (element, layer, scene) {
    ...
    if (cachedJSONDoubleBaseUnit1000) {
      let loader = new ObjectLoader();
      let object = loader.parse(cachedJSONDoubleBaseUnit1000);
      return Promise.resolve(onLoadItem(object));
    }

    return loadObjWithMaterial(mtl, obj, path.dirname(img1) + '/')
      .then(object => {
        cachedJSONDoubleBaseUnit1000 = object.toJSON();
        let loader = new ObjectLoader();
        return onLoadItem(loader.parse(cachedJSONDoubleBaseUnit1000))
      });
