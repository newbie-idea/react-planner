...
import {sceneValues, dwu1050GableArrays} from '../../utils/mtl-exports';

let globalTexture;
let worktopTexture;
let dwu1050GableLeftValue;
let dwu1050GableRightValue;
let pelmetValue;
let corniceValue;
let hingeNumber;

const mtl = require('./double-wall-unit-1050.mtl');
const obj = require('./double-wall-unit-1050.obj');
...
let cachedJSONDoubleWallUnit1050 = null;

export default {
  name: 'Double Wall Unit 1050',
  prototype: 'items',

  info: {
    title: 'DWU 1050',
    tag: ['double'],
    description: 'Double Wall Unit 1050',
    image: require('../../images/DWU-1050.png')
  },
  ...
  render2D: function (element, layer, scene) {
    ...
    dwu1050GableLeftValue = element.properties.get('gableLeft');
    dwu1050GableRightValue = element.properties.get('gableRight');
    dwu1050GableArrays(dwu1050GableLeftValue, dwu1050GableRightValue);
    ...
    if (true == dwu1050GableLeftValue) {
	    lineStyleB = { stroke: '#000', strokeWidth: '1px' };
    }
    
    if (true == dwu1050GableRightValue) {
  ...
  render3D: function (element, layer, scene) {
    ...
    if (cachedJSONDoubleWallUnit1050) {
      let loader = new ObjectLoader();
      let object = loader.parse(cachedJSONDoubleWallUnit1050);
      return Promise.resolve(onLoadItem(object));
    }

    return loadObjWithMaterial(mtl, obj, path.dirname(img1) + '/')
      .then(object => {
        cachedJSONDoubleWallUnit1050 = object.toJSON();
        let loader = new ObjectLoader();
        return onLoadItem(loader.parse(cachedJSONDoubleWallUnit1050))
      });
