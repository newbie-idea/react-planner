...
const width = {length: 1250, unit: 'mm'};
const depth = {length: 590, unit: 'mm'};
const height = {length: 870, unit: 'mm'};

let cachedJSONBlankEndCornerBaseUnit1250 = null;

export default {
  name: 'Blank End Corner Base Unit 1250',
  prototype: 'items',

  info: {
    title: 'BECBU 1250',
    tag: ['blank end', 'base'],
    description: 'Blank End Corner Base Unit 1250',
    image: require('../../images/BECBU-1250.png')
  },
  ...
  render3D: function (element, layer, scene) {
    ...
    if (cachedJSONBlankEndCornerBaseUnit1250) {
      let loader = new ObjectLoader();
      let object = loader.parse(cachedJSONBlankEndCornerBaseUnit1250);
      return Promise.resolve(onLoadItem(object));
    }

    return loadObjWithMaterial(mtl, obj, path.dirname(img1) + '/')
      .then(object => {
        cachedJSONBlankEndCornerBaseUnit1250 = object.toJSON();
        let loader = new ObjectLoader();
        return onLoadItem(loader.parse(cachedJSONBlankEndCornerBaseUnit1250))
      });
