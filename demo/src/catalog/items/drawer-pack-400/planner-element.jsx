...
import {sceneValues, dp400GableArrays} from '../../utils/mtl-exports';

let globalTexture;
let worktopTexture;
let dp400GableLeftValue;
let dp400GableRightValue;
let kickboardValue;

const mtl = require('./drawer-pack-400.mtl');
const obj = require('./drawer-pack-400.obj');
...
let cachedJSONDrawerPack400 = null;

export default {
  name: 'Drawer Pack 400',
  prototype: 'items',

  info: {
    title: 'DP 400',
    tag: ['single', 'base'],
    description: 'Drawer Pack 400',
    image: require('../../images/DP-400.png')
  },
  ...
  render2D: function (element, layer, scene) {
    ...
    dp400GableLeftValue = element.properties.get('gableLeft');
    dp400GableRightValue = element.properties.get('gableRight');
    dp400GableArrays(dp400GableLeftValue, dp400GableRightValue);
    ...
    if (true == dp400GableLeftValue) {
	    lineStyleB = { stroke: '#000', strokeWidth: '1px' };
    }
    
    if (true == dp400GableRightValue) {
  ...
  render3D: function (element, layer, scene) {
    ...
    if (cachedJSONDrawerPack400) {
      let loader = new ObjectLoader();
      let object = loader.parse(cachedJSONDrawerPack400);
      return Promise.resolve(onLoadItem(object));
    }

    return loadObjWithMaterial(mtl, obj, path.dirname(img1) + '/')
      .then(object => {
        cachedJSONDrawerPack400 = object.toJSON();
        let loader = new ObjectLoader();
        return onLoadItem(loader.parse(cachedJSONDrawerPack400))
      });
