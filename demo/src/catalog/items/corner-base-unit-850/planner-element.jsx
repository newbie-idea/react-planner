...
import {sceneValues, cbu850GableArrays} from '../../utils/mtl-exports';

let globalTexture;
let worktopTexture;
let cbu850GableLeftValue;
let cbu850GableRightValue;
let kickboardValue;
let hingeNumber;

const mtl = require('./corner-base-unit-850.mtl');
const obj = require('./corner-base-unit-850.obj');
...
const depth = {length: 850, unit: 'mm'};
const height = {length: 870, unit: 'mm'};

let cachedJSONCornerBaseUnit850 = null;

export default {
  name: 'Corner Base Unit 850',
  prototype: 'items',

  info: {
    title: 'CBU 850',
    tag: ['corner', 'base'],
    description: 'Corner Base Unit 850',
    image: require('../../images/CBU-850.png')
  },
  ...
  render2D: function (element, layer, scene) {
    ...
    cbu850GableLeftValue = element.properties.get('gableLeft');
    cbu850GableRightValue = element.properties.get('gableRight');
    cbu850GableArrays(cbu850GableLeftValue, cbu850GableRightValue);
    ...
    if (true == cbu850GableLeftValue) {
	    lineStyleB = { stroke: '#000', strokeWidth: '1px' };
    }
    
    if (true == cbu850GableRightValue) {
  ...
  render3D: function (element, layer, scene) {
    ...
    if (cachedJSONCornerBaseUnit850) {
      let loader = new ObjectLoader();
      let object = loader.parse(cachedJSONCornerBaseUnit850);
      return Promise.resolve(onLoadItem(object));
    }

    return loadObjWithMaterial(mtl, obj, path.dirname(img1) + '/')
      .then(object => {
        cachedJSONCornerBaseUnit850 = object.toJSON();
        let loader = new ObjectLoader();
        return onLoadItem(loader.parse(cachedJSONCornerBaseUnit850))
      });
