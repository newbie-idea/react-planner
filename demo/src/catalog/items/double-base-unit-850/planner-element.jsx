...
import {sceneValues, dbu850GableArrays} from '../../utils/mtl-exports';

let globalTexture;
let worktopTexture;
let dbu850GableLeftValue;
let dbu850GableRightValue;
let kickboardValue;
let hingeNumber;

const mtl = require('./double-base-unit-850.mtl');
const obj = require('./double-base-unit-850.obj');
...
let cachedJSONDoubleBaseUnit850 = null;

export default {
  name: 'Double Base Unit 850',
  prototype: 'items',

  info: {
    title: 'DBU 850',
    tag: ['double', 'base'],
    description: 'Double Base Unit 850',
    image: require('../../images/DBU-850.png')
  },
  ...
  render2D: function (element, layer, scene) {
    ...
    dbu850GableLeftValue = element.properties.get('gableLeft');
    dbu850GableRightValue = element.properties.get('gableRight');
    dbu850GableArrays(dbu850GableLeftValue, dbu850GableRightValue);
    ...
    if (true == dbu850GableLeftValue) {
	    lineStyleB = { stroke: '#000', strokeWidth: '1px' };
    }
    
    if (true == dbu850GableRightValue) {
  ...
  render3D: function (element, layer, scene) {
    ...
    if (cachedJSONDoubleBaseUnit850) {
      let loader = new ObjectLoader();
      let object = loader.parse(cachedJSONDoubleBaseUnit850);
      return Promise.resolve(onLoadItem(object));
    }

    return loadObjWithMaterial(mtl, obj, path.dirname(img1) + '/')
      .then(object => {
        cachedJSONDoubleBaseUnit850 = object.toJSON();
        let loader = new ObjectLoader();
        return onLoadItem(loader.parse(cachedJSONDoubleBaseUnit850))
      });
