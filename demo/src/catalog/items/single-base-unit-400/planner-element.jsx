...
import {sceneValues, sbu400GableArrays} from '../../utils/mtl-exports';

let globalTexture;
let worktopTexture;
let sbu400GableLeftValue;
let sbu400GableRightValue;
let kickboardValue;
let hingeNumber;

const mtl = require('./single-base-unit-400.mtl');
const obj = require('./single-base-unit-400.obj');
...
let cachedJSONSingleBaseUnit400 = null;

export default {
  name: 'Single Base Unit 400',
  prototype: 'items',

  info: {
    title: 'SBU 400',
    tag: ['single', 'base'],
    description: 'Single Base Unit 400',
    image: require('../../images/SBU-400.png')
  },
  ...
  render2D: function (element, layer, scene) {
    ...
    sbu400GableLeftValue = element.properties.get('gableLeft');
    sbu400GableRightValue = element.properties.get('gableRight');
    sbu400GableArrays(sbu400GableLeftValue, sbu400GableRightValue);
    ...
    if (true == sbu400GableLeftValue) {
	    lineStyleB = { stroke: '#000', strokeWidth: '1px' };
    }
    
    if (true == sbu400GableRightValue) {
  ...
  render3D: function (element, layer, scene) {
    ...
    if (cachedJSONSingleBaseUnit400) {
      let loader = new ObjectLoader();
      let object = loader.parse(cachedJSONSingleBaseUnit400);
      return Promise.resolve(onLoadItem(object));
    }

    return loadObjWithMaterial(mtl, obj, path.dirname(img1) + '/')
      .then(object => {
        cachedJSONSingleBaseUnit400 = object.toJSON();
        let loader = new ObjectLoader();
        return onLoadItem(loader.parse(cachedJSONSingleBaseUnit400))
      });
