...
import {sceneValues, becwu700GableArrays} from '../../utils/mtl-exports';

let globalTexture;
let worktopTexture;
let becwu700GableLeftValue;
let becwu700GableRightValue;
let pelmetValue;
let corniceValue;
let hingeNumber;
let doorNumber;

const mtl = require('./double-wall-unit-700.mtl');
const obj = require('./double-wall-unit-700.obj');
...
const width = {length: 700, unit: 'mm'};
const depth = {length: 300, unit: 'mm'};
const height = {length: 720, unit: 'mm'};

let cachedJSONBlankEndCornerWallUnit700 = null;

export default {
  name: 'Blank End Corner Wall Unit 700',
  prototype: 'items',

  info: {
    title: 'BECWU 700',
    tag: ['blank end', 'wall'],
    description: 'Blank End Corner Wall Unit 700',
    image: require('../../images/BECWU-700.png')
  },
  ...
  render2D: function (element, layer, scene) {
    ...
    becwu700GableLeftValue = element.properties.get('gableLeft');
    becwu700GableRightValue = element.properties.get('gableRight');
    becwu700GableArrays(becwu700GableLeftValue, becwu700GableRightValue);
    ...
    if (true == becwu700GableLeftValue) {
	    lineStyleB = { stroke: '#000', strokeWidth: '1px' };
    }
    
    if (true == becwu700GableRightValue) {
  ...
  render3D: function (element, layer, scene) {
    ...
    if (cachedJSONBlankEndCornerWallUnit700) {
      let loader = new ObjectLoader();
      let object = loader.parse(cachedJSONBlankEndCornerWallUnit700);
      return Promise.resolve(onLoadItem(object));
    }

    return loadObjWithMaterial(mtl, obj, path.dirname(img1) + '/')
      .then(object => {
        cachedJSONBlankEndCornerWallUnit700 = object.toJSON();
        let loader = new ObjectLoader();
        return onLoadItem(loader.parse(cachedJSONBlankEndCornerWallUnit700))
      });
