...
import {sceneValues, dp300GableArrays} from '../../utils/mtl-exports';

let globalTexture;
let worktopTexture;
let dp300GableLeftValue;
let dp300GableRightValue;
let kickboardValue;

const mtl = require('./drawer-pack-300.mtl');
const obj = require('./drawer-pack-300.obj');
...
let cachedJSONDrawerPack300 = null;

export default {
  name: 'Drawer Pack 300',
  prototype: 'items',

  info: {
    title: 'DP 300',
    tag: ['single', 'base'],
    description: 'Drawer Pack 300',
    image: require('../../images/DP-300.png')
  },
  ...
  render2D: function (element, layer, scene) {
    ...
    dp300GableLeftValue = element.properties.get('gableLeft');
    dp300GableRightValue = element.properties.get('gableRight');
    dp300GableArrays(dp300GableLeftValue, dp300GableRightValue);
    ...
    if (true == dp300GableLeftValue) {
	    lineStyleB = { stroke: '#000', strokeWidth: '1px' };
    }
    
    if (true == dp300GableRightValue) {
  ...
  render3D: function (element, layer, scene) {
    ...
    if (cachedJSONDrawerPack300) {
      let loader = new ObjectLoader();
      let object = loader.parse(cachedJSONDrawerPack300);
      return Promise.resolve(onLoadItem(object));
    }

    return loadObjWithMaterial(mtl, obj, path.dirname(img1) + '/')
      .then(object => {
        cachedJSONDrawerPack300 = object.toJSON();
        let loader = new ObjectLoader();
        return onLoadItem(loader.parse(cachedJSONDrawerPack300))
      });
