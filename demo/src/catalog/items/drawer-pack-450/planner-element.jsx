...
import {sceneValues, dp450GableArrays} from '../../utils/mtl-exports';

let globalTexture;
let worktopTexture;
let dp450GableLeftValue;
let dp450GableRightValue;
let kickboardValue;

const mtl = require('./drawer-pack-450.mtl');
const obj = require('./drawer-pack-450.obj');
...
let cachedJSONDrawerPack450 = null;

export default {
  name: 'Drawer Pack 450',
  prototype: 'items',

  info: {
    title: 'DP 450',
    tag: ['single'],
    description: 'Drawer Pack 450',
    image: require('../../images/DP-450.png')
  },
  ...
  render2D: function (element, layer, scene) {
    ...
    dp450GableLeftValue = element.properties.get('gableLeft');
    dp450GableRightValue = element.properties.get('gableRight');
    dp450GableArrays(dp450GableLeftValue, dp450GableRightValue);
    ...
    if (true == dp450GableLeftValue) {
	    lineStyleB = { stroke: '#000', strokeWidth: '1px' };
    }
    
    if (true == dp450GableRightValue) {
  ...
  render3D: function (element, layer, scene) {
    ...
    if (cachedJSONDrawerPack450) {
      let loader = new ObjectLoader();
      let object = loader.parse(cachedJSONDrawerPack450);
      return Promise.resolve(onLoadItem(object));
    }

    return loadObjWithMaterial(mtl, obj, path.dirname(img1) + '/')
      .then(object => {
        cachedJSONDrawerPack450 = object.toJSON();
        let loader = new ObjectLoader();
        return onLoadItem(loader.parse(cachedJSONDrawerPack450))
      });
