...
import {sceneValues, becbu1100GableArrays} from '../../utils/mtl-exports';

let globalTexture;
let worktopTexture;
let becbu1100GableLeftValue;
let becbu1100GableRightValue;
let kickboardValue;
let hingeNumber;
let doorNumber;

const mtl = require('./double-base-unit-1100.mtl');
const obj = require('./double-base-unit-1100.obj');
...
const width = {length: 1100, unit: 'mm'};
const depth = {length: 590, unit: 'mm'};
const height = {length: 870, unit: 'mm'};

let cachedJSONBlankEndCornerBaseUnit1100 = null;

export default {
  name: 'Blank End Corner Base Unit 1100',
  prototype: 'items',

  info: {
    title: 'BECBU 1100',
    tag: ['blank end', 'base'],
    description: 'Blank End Corner Base Unit 1100',
    image: require('../../images/BECBU-1100.png')
  },
  ...
  render2D: function (element, layer, scene) {
    ...
    becbu1100GableLeftValue = element.properties.get('gableLeft');
    becbu1100GableRightValue = element.properties.get('gableRight');
    becbu1100GableArrays(becbu1100GableLeftValue, becbu1100GableRightValue);
    ...
    if (true == becbu1100GableLeftValue) {
	    lineStyleB = { stroke: '#000', strokeWidth: '1px' };
    }
    
    if (true == becbu1100GableRightValue) {
  ...
  render3D: function (element, layer, scene) {
    ...
    if (cachedJSONBlankEndCornerBaseUnit1100) {
      let loader = new ObjectLoader();
      let object = loader.parse(cachedJSONBlankEndCornerBaseUnit1100);
      return Promise.resolve(onLoadItem(object));
    }

    return loadObjWithMaterial(mtl, obj, path.dirname(img1) + '/')
      .then(object => {
        cachedJSONBlankEndCornerBaseUnit1100 = object.toJSON();
        let loader = new ObjectLoader();
        return onLoadItem(loader.parse(cachedJSONBlankEndCornerBaseUnit1100))
      });
