...
import {sceneValues, cbu1150GableArrays} from '../../utils/mtl-exports';

let globalTexture;
let worktopTexture;
let cbu1150GableLeftValue;
let cbu1150GableRightValue;
let kickboardValue;
let hingeNumber;

const mtl = require('./corner-base-unit-1150.mtl');
const obj = require('./corner-base-unit-1150.obj');
...
const depth = {length: 1150, unit: 'mm'};
const height = {length: 870, unit: 'mm'};

let cachedJSONCornerBaseUnit1150 = null;

export default {
  name: 'Corner Base Unit 1150',
  prototype: 'items',

  info: {
    title: 'CBU 1150',
    tag: ['corner', 'base'],
    description: 'Corner Base Unit 1150',
    image: require('../../images/CBU-1150.png')
  },
  ...
  render2D: function (element, layer, scene) {
    ...
    cbu1150GableLeftValue = element.properties.get('gableLeft');
    cbu1150GableRightValue = element.properties.get('gableRight');
    cbu1150GableArrays(cbu1150GableLeftValue, cbu1150GableRightValue);
    ...
    if (true == cbu1150GableLeftValue) {
	    lineStyleB = { stroke: '#000', strokeWidth: '1px' };
    }
    
    if (true == cbu1150GableRightValue) {
  ...
  render3D: function (element, layer, scene) {
    ...
    if (cachedJSONCornerBaseUnit1150) {
      let loader = new ObjectLoader();
      let object = loader.parse(cachedJSONCornerBaseUnit1150);
      return Promise.resolve(onLoadItem(object));
    }

    return loadObjWithMaterial(mtl, obj, path.dirname(img1) + '/')
      .then(object => {
        cachedJSONCornerBaseUnit1150 = object.toJSON();
        let loader = new ObjectLoader();
        return onLoadItem(loader.parse(cachedJSONCornerBaseUnit1150))
      });
