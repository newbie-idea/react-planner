...
import {sceneValues, swu300GableArrays} from '../../utils/mtl-exports';

let globalTexture;
let worktopTexture;
let swu300GableLeftValue;
let swu300GableRightValue;
let pelmetValue;
let corniceValue;
let hingeNumber;

const mtl = require('./single-wall-unit-300.mtl');
const obj = require('./single-wall-unit-300.obj');
...
let cachedJSONSingleWallUnit300 = null;

export default {
  name: 'Single Wall Unit 300',
  prototype: 'items',

  info: {
    title: 'SWU 300',
    tag: ['single'],
    description: 'Single Wall Unit 300',
    image: require('../../images/SWU-300.png')
  },
  ...
  render2D: function (element, layer, scene) {
    ...
    swu300GableLeftValue = element.properties.get('gableLeft');
    swu300GableRightValue = element.properties.get('gableRight');
    swu300GableArrays(swu300GableLeftValue, swu300GableRightValue);
    ...
    if (true == swu300GableLeftValue) {
	    lineStyleB = { stroke: '#000', strokeWidth: '1px' };
    }
    
    if (true == swu300GableRightValue) {
  ...
  render3D: function (element, layer, scene) {
    ...
    if (cachedJSONSingleWallUnit300) {
      let loader = new ObjectLoader();
      let object = loader.parse(cachedJSONSingleWallUnit300);
      return Promise.resolve(onLoadItem(object));
    }

    return loadObjWithMaterial(mtl, obj, path.dirname(img1) + '/')
      .then(object => {
        cachedJSONSingleWallUnit300 = object.toJSON();
        let loader = new ObjectLoader();
        return onLoadItem(loader.parse(cachedJSONSingleWallUnit300))
      });
