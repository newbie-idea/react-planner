...
import {sceneValues, dbu900GableArrays} from '../../utils/mtl-exports';

let globalTexture;
let worktopTexture;
let dbu900GableLeftValue;
let dbu900GableRightValue;
let kickboardValue;
let hingeNumber;
...
let cachedJSONDoubleBaseUnit900 = null;

export default {
  name: 'Double Base Unit 900',
  prototype: 'items',

  info: {
    title: 'DBU 900',
    tag: ['double', 'base'],
    description: 'Double Base Unit 900',
    image: require('../../images/DBU-900.png')
  },
  ...
  render2D: function (element, layer, scene) {
    ...
    dbu900GableLeftValue = element.properties.get('gableLeft');
    dbu900GableRightValue = element.properties.get('gableRight');
    dbu900GableArrays(dbu900GableLeftValue, dbu900GableRightValue);
    ...
    if (true == dbu900GableLeftValue) {
	    lineStyleB = { stroke: '#000', strokeWidth: '1px' };
    }
    
    if (true == dbu900GableRightValue) {
  ...
  render3D: function (element, layer, scene) {
    ...
    if (cachedJSONDoubleBaseUnit900) {
      let loader = new ObjectLoader();
      let object = loader.parse(cachedJSONDoubleBaseUnit900);
      return Promise.resolve(onLoadItem(object));
    }

    return loadObjWithMaterial(mtl, obj, path.dirname(img1) + '/')
      .then(object => {
        cachedJSONDoubleBaseUnit900 = object.toJSON();
        let loader = new ObjectLoader();
        return onLoadItem(loader.parse(cachedJSONDoubleBaseUnit900))
      });
