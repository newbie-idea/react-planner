...
import {sceneValues, sbu500GableArrays} from '../../utils/mtl-exports';

let globalTexture;
let worktopTexture;
let sbu500GableLeftValue;
let sbu500GableRightValue;
let kickboardValue;
let hingeNumber;

const mtl = require('./single-base-unit-500.mtl');
const obj = require('./single-base-unit-500.obj');
...
let cachedJSONSingleBaseUnit500 = null;

export default {
  name: 'Single Base Unit 500',
  prototype: 'items',

  info: {
    title: 'SBU 500',
    tag: ['single', 'base'],
    description: 'Single Base Unit 500',
    image: require('../../images/SBU-500.png')
  },
  ...
  render2D: function (element, layer, scene) {
    ...
    sbu500GableLeftValue = element.properties.get('gableLeft');
    sbu500GableRightValue = element.properties.get('gableRight');
    sbu500GableArrays(sbu500GableLeftValue, sbu500GableRightValue);
    ...
    if (true == sbu500GableLeftValue) {
	    lineStyleB = { stroke: '#000', strokeWidth: '1px' };
    }
    
    if (true == sbu500GableRightValue) {
  ...
  render3D: function (element, layer, scene) {
    ...
    if (cachedJSONSingleBaseUnit500) {
      let loader = new ObjectLoader();
      let object = loader.parse(cachedJSONSingleBaseUnit500);
      return Promise.resolve(onLoadItem(object));
    }

    return loadObjWithMaterial(mtl, obj, path.dirname(img1) + '/')
      .then(object => {
        cachedJSONSingleBaseUnit500 = object.toJSON();
        let loader = new ObjectLoader();
        return onLoadItem(loader.parse(cachedJSONSingleBaseUnit500))
      });
