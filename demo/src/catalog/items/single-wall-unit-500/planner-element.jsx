...
import {sceneValues, swu500GableArrays} from '../../utils/mtl-exports';

let globalTexture;
let worktopTexture;
let swu500GableLeftValue;
let swu500GableRightValue;
let pelmetValue;
let corniceValue;
let hingeNumber;

const mtl = require('./single-wall-unit-500.mtl');
const obj = require('./single-wall-unit-500.obj');
...
let cachedJSONSingleWallUnit500 = null;

export default {
  name: 'Single Wall Unit 500',
  prototype: 'items',

  info: {
    title: 'SWU 500',
    tag: ['single', 'wall'],
    description: 'Single Wall Unit 500',
    image: require('../../images/SWU-500.png')
  },
  ...
  render2D: function (element, layer, scene) {
    ...
    swu500GableLeftValue = element.properties.get('gableLeft');
    swu500GableRightValue = element.properties.get('gableRight');
    swu500GableArrays(swu500GableLeftValue, swu500GableRightValue);
    ...
    if (true == swu500GableLeftValue) {
	    lineStyleB = { stroke: '#000', strokeWidth: '1px' };
    }
    
    if (true == swu500GableRightValue) {
  ...
  render3D: function (element, layer, scene) {
    ...
    if (cachedJSONSingleWallUnit500) {
      let loader = new ObjectLoader();
      let object = loader.parse(cachedJSONSingleWallUnit500);
      return Promise.resolve(onLoadItem(object));
    }

    return loadObjWithMaterial(mtl, obj, path.dirname(img1) + '/')
      .then(object => {
        cachedJSONSingleWallUnit500 = object.toJSON();
        let loader = new ObjectLoader();
        return onLoadItem(loader.parse(cachedJSONSingleWallUnit500))
      });
