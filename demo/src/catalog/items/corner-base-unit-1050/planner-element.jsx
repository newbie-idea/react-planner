...
import {hinges, textOffsetBD, xOffset1, xOffset2, doorOffset1, kickboardOffset, doorOffset2, textOffset1, textOffset2} from '../../utils/obj-exports';
import {sceneValues, cbu1050GableArrays} from '../../utils/mtl-exports';

let globalTexture;
let worktopTexture;
let cbu1050GableLeftValue;
let cbu1050GableRightValue;
let kickboardValue;
let hingeNumber;

const mtl = require('./corner-base-unit-1050.mtl');
const obj = require('./corner-base-unit-1050.obj');
...
const width = {length: 1050, unit: 'mm'};
const depth = {length: 1050, unit: 'mm'};
const height = {length: 870, unit: 'mm'};

let cachedJSONCornerBaseUnit1050 = null;

export default {
  name: 'Corner Base Unit 1050',
  prototype: 'items',

  info: {
    title: 'CBU 1050',
    tag: ['corner', 'base'],
    description: 'Corner Base Unit 1050',
    image: require('../../images/CBU-1050.png')
  },
  ...
  render2D: function (element, layer, scene) {
    ...
    cbu1050GableLeftValue = element.properties.get('gableLeft');
    cbu1050GableRightValue = element.properties.get('gableRight');
    cbu1050GableArrays(cbu1050GableLeftValue, cbu1050GableRightValue);
    ...
    if (true == cbu1050GableLeftValue) {
	    lineStyleB = { stroke: '#000', strokeWidth: '1px' };
    }
    
    if (true == cbu1050GableRightValue) {
  ...
  render3D: function (element, layer, scene) {
    ...
    if (cachedJSONCornerBaseUnit1050) {
      let loader = new ObjectLoader();
      let object = loader.parse(cachedJSONCornerBaseUnit1050);
      return Promise.resolve(onLoadItem(object));
    }

    return loadObjWithMaterial(mtl, obj, path.dirname(img1) + '/')
      .then(object => {
        cachedJSONCornerBaseUnit1050 = object.toJSON();
        let loader = new ObjectLoader();
        return onLoadItem(loader.parse(cachedJSONCornerBaseUnit1050))
      });
