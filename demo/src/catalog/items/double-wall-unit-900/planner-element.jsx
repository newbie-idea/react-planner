...
import {sceneValues, dwu900GableArrays} from '../../utils/mtl-exports';

let globalTexture;
let worktopTexture;
let dwu900GableLeftValue;
let dwu900GableRightValue;
let pelmetValue;
let corniceValue;
let hingeNumber;
...
let cachedJSONDoubleWallUnit900 = null;

export default {
  name: 'Double Wall Unit 900',
  prototype: 'items',

  info: {
    title: 'DWU 900',
    tag: ['double', 'wall'],
    description: 'Double Wall Unit 900',
    image: require('../../images/DWU-900.png')
  },
  ...
  render2D: function (element, layer, scene) {
    ...
    dwu900GableLeftValue = element.properties.get('gableLeft');
    dwu900GableRightValue = element.properties.get('gableRight');
    dwu900GableArrays(dwu900GableLeftValue, dwu900GableRightValue);
    ...
    if (true == dwu900GableLeftValue) {
	    lineStyleB = { stroke: '#000', strokeWidth: '1px' };
    }
    
    if (true == dwu900GableRightValue) {
  ...
  render3D: function (element, layer, scene) {
    ...
    if (cachedJSONDoubleWallUnit900) {
      let loader = new ObjectLoader();
      let object = loader.parse(cachedJSONDoubleWallUnit900);
      return Promise.resolve(onLoadItem(object));
    }

    return loadObjWithMaterial(mtl, obj, path.dirname(img1) + '/')
      .then(object => {
        cachedJSONDoubleWallUnit900 = object.toJSON();
        let loader = new ObjectLoader();
        return onLoadItem(loader.parse(cachedJSONDoubleWallUnit900))
      });
