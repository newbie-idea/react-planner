...
import {sceneValues, dwu800GableArrays} from '../../utils/mtl-exports';

let globalTexture;
let worktopTexture;
let dwu800GableLeftValue;
let dwu800GableRightValue;
let pelmetValue;
let corniceValue;
let hingeNumber;
...
let cachedJSONDoubleWallUnit800 = null;

export default {
  name: 'Double Wall Unit 800',
  prototype: 'items',

  info: {
    title: 'DWU 800',
    tag: ['double', 'wall'],
    description: 'Double Wall Unit 800',
    image: require('../../images/DWU-800.png')
  },
  ...
  render2D: function (element, layer, scene) {
    ...
    dwu800GableLeftValue = element.properties.get('gableLeft');
    dwu800GableRightValue = element.properties.get('gableRight');
    dwu800GableArrays(dwu800GableLeftValue, dwu800GableRightValue);
    ...
    if (true == dwu800GableLeftValue) {
	    lineStyleB = { stroke: '#000', strokeWidth: '1px' };
    }
    
    if (true == dwu800GableRightValue) {
  ...
  render3D: function (element, layer, scene) {
    ...
    if (cachedJSONDoubleWallUnit800) {
      let loader = new ObjectLoader();
      let object = loader.parse(cachedJSONDoubleWallUnit800);
      return Promise.resolve(onLoadItem(object));
    }

    return loadObjWithMaterial(mtl, obj, path.dirname(img1) + '/')
      .then(object => {
        cachedJSONDoubleWallUnit800 = object.toJSON();
        let loader = new ObjectLoader();
        return onLoadItem(loader.parse(cachedJSONDoubleWallUnit800))
      });
