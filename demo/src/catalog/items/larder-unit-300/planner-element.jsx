...
import {sceneValues, lu300GableArrays} from '../../utils/mtl-exports';

let globalTexture;
let worktopTexture;
let lu300GableLeftValue;
let lu300GableRightValue;
let kickboardValue;
let hingeNumber;

const mtl = require('./larder-unit-300.mtl');
const obj = require('./larder-unit-300.obj');
...
const width = {length: 300, unit: 'mm'};
const depth = {length: 590, unit: 'mm'};
const height = {length: 2120, unit: 'mm'};

let cachedJSONLarderUnit300 = null;

export default {
  name: 'Larder Unit 300',
  prototype: 'items',

  info: {
    title: 'LU 300',
    tag: ['single', 'base'],
    description: 'Larder Unit 300',
    image: require('../../images/LU-300.png')
  },
  ...
  render2D: function (element, layer, scene) {
    ...
    lu300GableLeftValue = element.properties.get('gableLeft');
    lu300GableRightValue = element.properties.get('gableRight');
    lu300GableArrays(lu300GableLeftValue, lu300GableRightValue);
    ...
    if (true == lu300GableLeftValue) {
	    lineStyleB = { stroke: '#000', strokeWidth: '1px' };
    }
    
    if (true == lu300GableRightValue) {
  ...
  render3D: function (element, layer, scene) {
    ...
    if (cachedJSONLarderUnit300) {
      let loader = new ObjectLoader();
      let object = loader.parse(cachedJSONLarderUnit300);
      return Promise.resolve(onLoadItem(object));
    }

    return loadObjWithMaterial(mtl, obj, path.dirname(img1) + '/')
      .then(object => {
        cachedJSONLarderUnit300 = object.toJSON();
        let loader = new ObjectLoader();
        return onLoadItem(loader.parse(cachedJSONLarderUnit300))
      });
