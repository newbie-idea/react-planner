...
import {sceneValues, ts200GableArrays} from '../../utils/mtl-exports';

let globalTexture;
let worktopTexture;
let ts200GableLeftValue;
let ts200GableRightValue;
let kickboardValue;
let hingeNumber;

const mtl = require('./tray-space-200.mtl');
const obj = require('./tray-space-200.obj');
...
let cachedJSONTraySpace200 = null;
...
export default {
  ...
  info: {
    title: 'TS 200',
    tag: ['single', 'base'],
    description: 'Tray Space 200',
    image: require('../../images/TS-200.png')
  },
  ...
  render2D: function (element, layer, scene) {
    ...
    ts200GableLeftValue = element.properties.get('gableLeft');
    ts200GableRightValue = element.properties.get('gableRight');
    ts200GableArrays(ts200GableLeftValue, ts200GableRightValue);
    ...
    if (true == ts200GableLeftValue) {
	    lineStyleB = { stroke: '#000', strokeWidth: '1px' };
    }
    
    if (true == ts200GableRightValue) {
  ...
  render3D: function (element, layer, scene) {
    ...
    if (cachedJSONTraySpace200) {
      let loader = new ObjectLoader();
      let object = loader.parse(cachedJSONTraySpace200);
      return Promise.resolve(onLoadItem(object));
    }

    return loadObjWithMaterial(mtl, obj, path.dirname(img1) + '/')
      .then(object => {
        cachedJSONTraySpace200 = object.toJSON();
        let loader = new ObjectLoader();
        return onLoadItem(loader.parse(cachedJSONTraySpace200))
      });
