...
import {sceneValues, sbu600GableArrays} from '../../utils/mtl-exports';

let globalTexture;
let worktopTexture;
let sbu600GableLeftValue;
let sbu600GableRightValue;
let kickboardValue;
let hingeNumber;

const mtl = require('./single-base-unit-600.mtl');
const obj = require('./single-base-unit-600.obj');
...
let cachedJSONSingleBaseUnit600 = null;

export default {
  name: 'Single Base Unit 600',
  prototype: 'items',

  info: {
    title: 'SBU 600',
    tag: ['single', 'base'],
    description: 'Single Base Unit 600',
    image: require('../../images/SBU-600.png')
  },
  ...
  render2D: function (element, layer, scene) {
    ...
    sbu600GableLeftValue = element.properties.get('gableLeft');
    sbu600GableRightValue = element.properties.get('gableRight');
    sbu600GableArrays(sbu600GableLeftValue, sbu600GableRightValue);
    ...
    if (true == sbu600GableLeftValue) {
	    lineStyleB = { stroke: '#000', strokeWidth: '1px' };
    }
    
    if (true == sbu600GableRightValue) {
  ...
  render3D: function (element, layer, scene) {
    ...
    if (cachedJSONSingleBaseUnit600) {
      let loader = new ObjectLoader();
      let object = loader.parse(cachedJSONSingleBaseUnit600);
      return Promise.resolve(onLoadItem(object));
    }

    return loadObjWithMaterial(mtl, obj, path.dirname(img1) + '/')
      .then(object => {
        cachedJSONSingleBaseUnit600 = object.toJSON();
        let loader = new ObjectLoader();
        return onLoadItem(loader.parse(cachedJSONSingleBaseUnit600))
      });
