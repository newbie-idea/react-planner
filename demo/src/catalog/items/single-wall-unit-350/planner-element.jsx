...
import {sceneValues, swu350GableArrays} from '../../utils/mtl-exports';

let globalTexture;
let worktopTexture;
let swu350GableLeftValue;
let swu350GableRightValue;
let pelmetValue;
let corniceValue;
let hingeNumber;

const mtl = require('./single-wall-unit-350.mtl');
const obj = require('./single-wall-unit-350.obj');
...
let cachedJSONSingleWallUnit350 = null;

export default {
  name: 'Single Wall Unit 350',
  prototype: 'items',

  info: {
    title: 'SWU 350',
    tag: ['single', 'wall'],
    description: 'Single Wall Unit 350',
    image: require('../../images/SWU-350.png')
  },
  ...
  render2D: function (element, layer, scene) {
    ...
    swu350GableLeftValue = element.properties.get('gableLeft');
    swu350GableRightValue = element.properties.get('gableRight');
    swu350GableArrays(swu350GableLeftValue, swu350GableRightValue);
    ...
    if (true == swu350GableLeftValue) {
	    lineStyleB = { stroke: '#000', strokeWidth: '1px' };
    }
    
    if (true == swu350GableRightValue) {
  ...
  render3D: function (element, layer, scene) {
    ...
    if (cachedJSONSingleWallUnit350) {
      let loader = new ObjectLoader();
      let object = loader.parse(cachedJSONSingleWallUnit350);
      return Promise.resolve(onLoadItem(object));
    }

    return loadObjWithMaterial(mtl, obj, path.dirname(img1) + '/')
      .then(object => {
        cachedJSONSingleWallUnit350 = object.toJSON();
        let loader = new ObjectLoader();
        return onLoadItem(loader.parse(cachedJSONSingleWallUnit350))
      });
