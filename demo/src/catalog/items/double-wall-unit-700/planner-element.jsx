...
import {sceneValues, dwu700GableArrays} from '../../utils/mtl-exports';

let globalTexture;
let worktopTexture;
let dwu700GableLeftValue;
let dwu700GableRightValue;
let pelmetValue;
let corniceValue;
let hingeNumber;
...
let cachedJSONDoubleWallUnit700 = null;

export default {
  name: 'Double Wall Unit 700',
  prototype: 'items',

  info: {
    title: 'DWU 700',
    tag: ['double', 'wall'],
    description: 'Double Wall Unit 700',
    image: require('../../images/DWU-700.png')
  },
  ...
  render2D: function (element, layer, scene) {
    ...
    dwu700GableLeftValue = element.properties.get('gableLeft');
    dwu700GableRightValue = element.properties.get('gableRight');
    dwu700GableArrays(dwu700GableLeftValue, dwu700GableRightValue);
    ...
    if (true == dwu700GableLeftValue) {
	    lineStyleB = { stroke: '#000', strokeWidth: '1px' };
    }
    
    if (true == dwu700GableRightValue) {
  ...
  render3D: function (element, layer, scene) {
    ...
    if (cachedJSONDoubleWallUnit700) {
      let loader = new ObjectLoader();
      let object = loader.parse(cachedJSONDoubleWallUnit700);
      return Promise.resolve(onLoadItem(object));
    }

    return loadObjWithMaterial(mtl, obj, path.dirname(img1) + '/')
      .then(object => {
        cachedJSONDoubleWallUnit700 = object.toJSON();
        let loader = new ObjectLoader();
        return onLoadItem(loader.parse(cachedJSONDoubleWallUnit700))
      });
