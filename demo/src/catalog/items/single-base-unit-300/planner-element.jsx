...
import {sceneValues, sbu300GableArrays} from '../../utils/mtl-exports';

let globalTexture;
let worktopTexture;
let sbu300GableLeftValue;
let sbu300GableRightValue;
let kickboardValue;
let hingeNumber;

const mtl = require('./single-base-unit-300.mtl');
const obj = require('./single-base-unit-300.obj');
...
let cachedJSONSingleBaseUnit300 = null;

export default {
  name: 'Single Base Unit 300',
  prototype: 'items',

  info: {
    title: 'SBU 300',
    tag: ['single', 'base'],
    description: 'Single Base Unit 300',
    image: require('../../images/SBU-300.png')
  },
  ...
  render2D: function (element, layer, scene) {
    ...
    sbu300GableLeftValue = element.properties.get('gableLeft');
    sbu300GableRightValue = element.properties.get('gableRight');
    sbu300GableArrays(sbu300GableLeftValue, sbu300GableRightValue);
    ...
    if (true == sbu300GableLeftValue) {
	    lineStyleB = { stroke: '#000', strokeWidth: '1px' };
    }
    
    if (true == sbu300GableRightValue) {
  ...
  render3D: function (element, layer, scene) {
    ...
    if (cachedJSONSingleBaseUnit300) {
      let loader = new ObjectLoader();
      let object = loader.parse(cachedJSONSingleBaseUnit300);
      return Promise.resolve(onLoadItem(object));
    }

    return loadObjWithMaterial(mtl, obj, path.dirname(img1) + '/')
      .then(object => {
        cachedJSONSingleBaseUnit300 = object.toJSON();
        let loader = new ObjectLoader();
        return onLoadItem(loader.parse(cachedJSONSingleBaseUnit300))
      });
