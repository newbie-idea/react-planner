...
const mtl = require('./cooker-housing-unit.mtl');
const obj = require('./cooker-housing-unit.obj');
...
let cachedJSONCookerHousingUnit = null;

export default {
  name: 'Cooking Housing Unit',
  prototype: 'items',

  info: {
    title: 'CHU',
    tag: ['single', 'base'],
    description: 'Cooker Housing Unit',
    image: require('../../images/CHU.png')
  },
  ...
  render2D: function (element, layer, scene) {
    ...
    let circleStyle = {stroke: element.selected ? '#0096fd' : '#000', strokeWidth: "1px", fill: fillValue};
    ...
    return (
      <g transform={`translate(${-newWidth / 1},${-newDepth / 1}) scale(2,2)`}>
        ...
        <circle key="2" cx={newWidth / 4} cy={newDepth / 4} r={RADIUS} style={circleStyle}/>
		    <circle key="3" cx={newWidth / 4 * 3} cy={newDepth / 4} r={RADIUS} style={circleStyle}/>
		    <circle key="4" cx={newWidth / 4} cy={newDepth / 4 * 3} r={RADIUS} style={circleStyle}/>
		    <circle key="5" cx={newWidth / 4 * 3} cy={newDepth / 4 * 3} r={RADIUS} style={circleStyle}/>
        <line 
		      x1={0}
		      y1={newDepth / 2}
		      x2={newWidth}
  ...
  render3D: function (element, layer, scene) {
    ...
    if (cachedJSONCookerHousingUnit) {
      let loader = new ObjectLoader();
      let object = loader.parse(cachedJSONCookerHousingUnit);
      return Promise.resolve(onLoadItem(object));
    }

    return loadObjWithMaterial(mtl, obj, path.dirname(img1) + '/')
      .then(object => {
        cachedJSONCookerHousingUnit = object.toJSON();
        let loader = new ObjectLoader();
        return onLoadItem(loader.parse(cachedJSONCookerHousingUnit))
      });
