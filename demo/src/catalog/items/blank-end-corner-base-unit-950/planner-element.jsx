...
import {sceneValues, becbu950GableArrays} from '../../utils/mtl-exports';

let globalTexture;
let worktopTexture;
let becbu950GableLeftValue;
let becbu950GableRightValue;
let kickboardValue;
let hingeNumber;
let doorNumber;

const mtl = require('./double-base-unit-950.mtl');
const obj = require('./double-base-unit-950.obj');
...
const width = {length: 950, unit: 'mm'};
const depth = {length: 590, unit: 'mm'};
const height = {length: 870, unit: 'mm'};

let cachedJSONBlankEndCornerBaseUnit950 = null;

export default {
  name: 'Blank End Corner Base Unit 950',
  prototype: 'items',

  info: {
    title: 'BECBU 950',
    tag: ['blank end', 'base'],
    description: 'Blank End Corner Base Unit 950',
    image: require('../../images/BECBU-950.png')
  },
  ...
  render2D: function (element, layer, scene) {
    ...
    becbu950GableLeftValue = element.properties.get('gableLeft');
    becbu950GableRightValue = element.properties.get('gableRight');
    becbu950GableArrays(becbu950GableLeftValue, becbu950GableRightValue);
    ...
    if (true == becbu950GableLeftValue) {
	    lineStyleB = { stroke: '#000', strokeWidth: '1px' };
    }
    
    if (true == becbu950GableRightValue) {
  ...
  render3D: function (element, layer, scene) {
    ...
    if (cachedJSONDoubleBaseUnit950) {
      let loader = new ObjectLoader();
      let object = loader.parse(cachedJSONDoubleBaseUnit950);
      return Promise.resolve(onLoadItem(object));
    }

    return loadObjWithMaterial(mtl, obj, path.dirname(img1) + '/')
      .then(object => {
        cachedJSONDoubleBaseUnit950 = object.toJSON();
        let loader = new ObjectLoader();
        return onLoadItem(loader.parse(cachedJSONDoubleBaseUnit950))
      });
