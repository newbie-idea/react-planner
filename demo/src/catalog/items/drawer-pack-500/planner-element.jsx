...
import {sceneValues, dp500GableArrays} from '../../utils/mtl-exports';

let globalTexture;
let worktopTexture;
let dp500GableLeftValue;
let dp500GableRightValue;
let kickboardValue;

const mtl = require('./drawer-pack-500.mtl');
const obj = require('./drawer-pack-500.obj');
...
let cachedJSONDrawerPack500 = null;

export default {
  name: 'Drawer Pack 500',
  prototype: 'items',

  info: {
    title: 'DP 500',
    tag: ['single', 'base'],
    description: 'Drawer Pack 500',
    image: require('../../images/DP-500.png')
  },
  ...
  render2D: function (element, layer, scene) {
    ...
    dp500GableLeftValue = element.properties.get('gableLeft');
    dp500GableRightValue = element.properties.get('gableRight');
    dp500GableArrays(dp500GableLeftValue, dp500GableRightValue);
    ...
    if (true == dp500GableLeftValue) {
	    lineStyleB = { stroke: '#000', strokeWidth: '1px' };
    }
    
    if (true == dp500GableRightValue) {
  ...
  render3D: function (element, layer, scene) {
    ...
    if (cachedJSONDrawerPack500) {
      let loader = new ObjectLoader();
      let object = loader.parse(cachedJSONDrawerPack500);
      return Promise.resolve(onLoadItem(object));
    }

    return loadObjWithMaterial(mtl, obj, path.dirname(img1) + '/')
      .then(object => {
        cachedJSONDrawerPack500 = object.toJSON();
        let loader = new ObjectLoader();
        return onLoadItem(loader.parse(cachedJSONDrawerPack500))
      });
