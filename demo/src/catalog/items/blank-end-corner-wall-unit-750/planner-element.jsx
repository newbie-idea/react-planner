...
import {sceneValues, becwu750GableArrays} from '../../utils/mtl-exports';

let globalTexture;
let worktopTexture;
let becwu750GableLeftValue;
let becwu750GableRightValue;
let pelmetValue;
let corniceValue;
let hingeNumber;
let doorNumber;

const mtl = require('./double-wall-unit-750.mtl');
const obj = require('./double-wall-unit-750.obj');
...
let cachedJSONBlankEndCornerWallUnit750 = null;

export default {
  name: 'Blank End Wall Unit 750',
  prototype: 'items',

  info: {
    title: 'BECWU 750',
    tag: ['blank end', 'wall'],
    description: 'Blank End Corner Wall Unit 750',
    image: require('../../images/BECWU-750.png')
  },
  ...
  render2D: function (element, layer, scene) {
    ...
    becwu750GableLeftValue = element.properties.get('gableLeft');
    becwu750GableRightValue = element.properties.get('gableRight');
    becwu750GableArrays(becwu750GableLeftValue, becwu750GableRightValue);
    ...
    if (true == becwu750GableLeftValue) {
	    lineStyleB = { stroke: '#000', strokeWidth: '1px' };
    }
    
    if (true == becwu750GableRightValue) {
  ...
  render3D: function (element, layer, scene) {
    ...
    if (cachedJSONBlankEndCornerWallUnit750) {
      let loader = new ObjectLoader();
      let object = loader.parse(cachedJSONBlankEndCornerWallUnit750);
      return Promise.resolve(onLoadItem(object));
    }

    return loadObjWithMaterial(mtl, obj, path.dirname(img1) + '/')
      .then(object => {
        cachedJSONBlankEndCornerWallUnit750 = object.toJSON();
        let loader = new ObjectLoader();
        return onLoadItem(loader.parse(cachedJSONBlankEndCornerWallUnit750))
      });
