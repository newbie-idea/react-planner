...
import {sceneValues, becbu1150GableArrays} from '../../utils/mtl-exports';

let globalTexture;
let worktopTexture;
let becbu1150GableLeftValue;
let becbu1150GableRightValue;
let kickboardValue;
let hingeNumber;
let doorNumber;

const mtl = require('./double-base-unit-1150.mtl');
const obj = require('./double-base-unit-1150.obj');
...
const width = {length: 1150, unit: 'mm'};
const depth = {length: 590, unit: 'mm'};
const height = {length: 870, unit: 'mm'};

let cachedJSONBlankEndCornerBaseUnit1150 = null;

export default {
  name: 'Blank End Corner Base Unit 1150',
  prototype: 'items',

  info: {
    title: 'BECBU 1150',
    tag: ['blank end', 'base'],
    description: 'Blank End Corner Base Unit 1150',
    image: require('../../images/BECBU-1150.png')
  },
  ...
  render2D: function (element, layer, scene) {
    ...
    becbu1150GableLeftValue = element.properties.get('gableLeft');
    becbu1150GableRightValue = element.properties.get('gableRight');
    becbu1150GableArrays(becbu1150GableLeftValue, becbu1150GableRightValue);
    ...
    if (true == becbu1150GableLeftValue) {
	    lineStyleB = { stroke: '#000', strokeWidth: '1px' };
    }
    
    if (true == becbu1150GableRightValue) {
  ...
  render3D: function (element, layer, scene) {
    ...
    if (cachedJSONBlankEndCornerBaseUnit1150) {
      let loader = new ObjectLoader();
      let object = loader.parse(cachedJSONBlankEndCornerBaseUnit1150);
      return Promise.resolve(onLoadItem(object));
    }

    return loadObjWithMaterial(mtl, obj, path.dirname(img1) + '/')
      .then(object => {
        cachedJSONBlankEndCornerBaseUnit1150 = object.toJSON();
        let loader = new ObjectLoader();
        return onLoadItem(loader.parse(cachedJSONBlankEndCornerBaseUnit1150))
      });
