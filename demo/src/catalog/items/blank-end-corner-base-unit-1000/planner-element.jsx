...
import {sceneValues, becbu1000GableArrays} from '../../utils/mtl-exports';

let globalTexture;
let worktopTexture;
let becbu1000GableLeftValue;
let becbu1000GableRightValue;
let kickboardValue;
let hingeNumber;
let doorNumber;

const mtl = require('./double-base-unit-1000.mtl');
const obj = require('./double-base-unit-1000.obj');
...
const width = {length: 1000, unit: 'mm'};
const depth = {length: 590, unit: 'mm'};
const height = {length: 870, unit: 'mm'};

let cachedJSONBlankEndCornerBaseUnit1000 = null;

export default {
  name: 'Blank End Corner Base Unit 1000',
  prototype: 'items',

  info: {
    title: 'BECBU 1000',
    tag: ['blank end', 'base'],
    description: 'Blank End Corner Base Unit 1000',
    image: require('../../images/BECBU-1000.png')
  },
  ...
  render2D: function (element, layer, scene) {
    ...
    becbu1000GableLeftValue = element.properties.get('gableLeft');
    becbu1000GableRightValue = element.properties.get('gableRight');
    becbu1000GableArrays(becbu1000GableLeftValue, becbu1000GableRightValue);
    ...
    if (true == becbu1000GableLeftValue) {
	    lineStyleB = { stroke: '#000', strokeWidth: '1px' };
    }
    
    if (true == becbu1000GableRightValue) {
  ...
  render3D: function (element, layer, scene) {
    ...
    if (cachedJSONBlankEndCornerBaseUnit1000) {
      let loader = new ObjectLoader();
      let object = loader.parse(cachedJSONBlankEndCornerBaseUnit1000);
      return Promise.resolve(onLoadItem(object));
    }

    return loadObjWithMaterial(mtl, obj, path.dirname(img1) + '/')
      .then(object => {
        cachedJSONBlankEndCornerBaseUnit1000 = object.toJSON();
        let loader = new ObjectLoader();
        return onLoadItem(loader.parse(cachedJSONBlankEndCornerBaseUnit1000))
      });
