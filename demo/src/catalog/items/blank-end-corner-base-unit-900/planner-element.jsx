...
import {sceneValues, becbu900GableArrays} from '../../utils/mtl-exports';

let globalTexture;
let worktopTexture;
let becbu900GableLeftValue;
let becbu900GableRightValue;
let kickboardValue;
let hingeNumber;
let doorNumber;

const mtl = require('./double-base-unit-900.mtl');
const obj = require('./double-base-unit-900.obj');
...
let cachedJSONBlankEndCornerBaseUnit900 = null;

export default {
  name: 'Blank End Corner Base Unit 900',
  prototype: 'items',

  info: {
    title: 'BECBU 900',
    tag: ['blank end', 'base'],
    description: 'Blank End Corner Base Unit 900',
    image: require('../../images/BECBU-900.png')
  },
  ...
  render2D: function (element, layer, scene) {
    ...
    becbu900GableLeftValue = element.properties.get('gableLeft');
    becbu900GableRightValue = element.properties.get('gableRight');
    becbu900GableArrays(becbu900GableLeftValue, becbu900GableRightValue);
    ...
    if (true == becbu900GableLeftValue) {
	    lineStyleB = { stroke: '#000', strokeWidth: '1px' };
    }
    
    if (true == becbu900GableRightValue) {
  ...
  render3D: function (element, layer, scene) {
    ...
    if (cachedJSONBlankEndCornerBaseUnit900) {
      let loader = new ObjectLoader();
      let object = loader.parse(cachedJSONBlankEndCornerBaseUnit900);
      return Promise.resolve(onLoadItem(object));
    }

    return loadObjWithMaterial(mtl, obj, path.dirname(img1) + '/')
      .then(object => {
        cachedJSONBlankEndCornerBaseUnit900 = object.toJSON();
        let loader = new ObjectLoader();
        return onLoadItem(loader.parse(cachedJSONBlankEndCornerBaseUnit900))
      });
