...
import {sceneValues, sbu450GableArrays} from '../../utils/mtl-exports';

let globalTexture;
let worktopTexture;
let sbu450GableLeftValue;
let sbu450GableRightValue;
let kickboardValue;
let hingeNumber;

const mtl = require('./single-base-unit-450.mtl');
const obj = require('./single-base-unit-450.obj');
...
let cachedJSONSingleBaseUnit450 = null;

export default {
  name: 'Single Base Unit 450',
  prototype: 'items',

  info: {
    title: 'SBU 450',
    tag: ['single', 'base'],
    description: 'Single Base Unit 450',
    image: require('../../images/SBU-450.png')
  },
  ...
  render2D: function (element, layer, scene) {
    ...
    sbu450GableLeftValue = element.properties.get('gableLeft');
    sbu450GableRightValue = element.properties.get('gableRight');
    sbu450GableArrays(sbu450GableLeftValue, sbu450GableRightValue);
    ...
    if (true == sbu450GableLeftValue) {
	    lineStyleB = { stroke: '#000', strokeWidth: '1px' };
    }
    
    if (true == sbu450GableRightValue) {
  ...
  render3D: function (element, layer, scene) {
    ...
    if (cachedJSONSingleBaseUnit450) {
      let loader = new ObjectLoader();
      let object = loader.parse(cachedJSONSingleBaseUnit450);
      return Promise.resolve(onLoadItem(object));
    }

    return loadObjWithMaterial(mtl, obj, path.dirname(img1) + '/')
      .then(object => {
        cachedJSONSingleBaseUnit450 = object.toJSON();
        let loader = new ObjectLoader();
        return onLoadItem(loader.parse(cachedJSONSingleBaseUnit450))
      });
