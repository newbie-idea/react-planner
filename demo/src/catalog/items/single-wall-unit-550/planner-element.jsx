...
import {sceneValues, swu550GableArrays} from '../../utils/mtl-exports';

let globalTexture;
let worktopTexture;
let swu550GableLeftValue;
let swu550GableRightValue;
let pelmetValue;
let corniceValue;
let hingeNumber;

const mtl = require('./single-wall-unit-550.mtl');
const obj = require('./single-wall-unit-550.obj');
...
let cachedJSONSingleWallUnit550 = null;

export default {
  name: 'Single Wall Unit 550',
  prototype: 'items',

  info: {
    title: 'SWU 550',
    tag: ['single', 'wall'],
    description: 'Single Wall Unit 550',
    image: require('../../images/SWU-550.png')
  },
  ...
  render2D: function (element, layer, scene) {
    ...
    swu550GableLeftValue = element.properties.get('gableLeft');
    swu550GableRightValue = element.properties.get('gableRight');
    swu550GableArrays(swu550GableLeftValue, swu550GableRightValue);
    ...
    if (true == swu550GableLeftValue) {
	    lineStyleB = { stroke: '#000', strokeWidth: '1px' };
    }
    
    if (true == swu550GableRightValue) {
  ...
  render3D: function (element, layer, scene) {
    ...
    if (cachedJSONSingleWallUnit550) {
      let loader = new ObjectLoader();
      let object = loader.parse(cachedJSONSingleWallUnit550);
      return Promise.resolve(onLoadItem(object));
    }

    return loadObjWithMaterial(mtl, obj, path.dirname(img1) + '/')
      .then(object => {
        cachedJSONSingleWallUnit550 = object.toJSON();
        let loader = new ObjectLoader();
        return onLoadItem(loader.parse(cachedJSONSingleWallUnit550))
      });
