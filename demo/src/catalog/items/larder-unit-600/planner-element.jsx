...
import {sceneValues, lu600GableArrays} from '../../utils/mtl-exports';

let globalTexture;
let worktopTexture;
let lu600GableLeftValue;
let lu600GableRightValue;
let kickboardValue;
let hingeNumber;

const mtl = require('./larder-unit-600.mtl');
const obj = require('./larder-unit-600.obj');
...
let cachedJSONLarderUnit600 = null;

export default {
  name: 'Larder Unit 600',
  prototype: 'items',

  info: {
    title: 'LU 600',
    tag: ['single', 'base'],
    description: 'Larder Unit 600',
    image: require('../../images/LU-600.png')
  },
  ...
  render2D: function (element, layer, scene) {
    ...
    lu600GableLeftValue = element.properties.get('gableLeft');
    lu600GableRightValue = element.properties.get('gableRight');
    lu600GableArrays(lu600GableLeftValue, lu600GableRightValue);
    ...
    if (true == lu600GableLeftValue) {
	    lineStyleB = { stroke: '#000', strokeWidth: '1px' };
    }
    
    if (true == lu600GableRightValue) {
  ...
  render3D: function (element, layer, scene) {
    ...
    if (cachedJSONLarderUnit600) {
      let loader = new ObjectLoader();
      let object = loader.parse(cachedJSONLarderUnit600);
      return Promise.resolve(onLoadItem(object));
    }

    return loadObjWithMaterial(mtl, obj, path.dirname(img1) + '/')
      .then(object => {
        cachedJSONLarderUnit600 = object.toJSON();
        let loader = new ObjectLoader();
        return onLoadItem(loader.parse(cachedJSONLarderUnit600))
      });
