...
import {sceneValues, lu450GableArrays} from '../../utils/mtl-exports';

let globalTexture;
let worktopTexture;
let lu450GableLeftValue;
let lu450GableRightValue;
let kickboardValue;
let hingeNumber;

const mtl = require('./larder-unit-450.mtl');
const obj = require('./larder-unit-450.obj');
...
const width = {length: 450, unit: 'mm'};
const depth = {length: 590, unit: 'mm'};
const height = {length: 2120, unit: 'mm'};

let cachedJSONLarderUnit450 = null;

export default {
  name: 'Larder Unit 450',
  prototype: 'items',

  info: {
    title: 'LU 450',
    tag: ['single', 'base'],
    description: 'Larder Unit 450',
    image: require('../../images/LU-450.png')
  },
  ...
  render2D: function (element, layer, scene) {
    ...
    lu450GableLeftValue = element.properties.get('gableLeft');
    lu450GableRightValue = element.properties.get('gableRight');
    lu450GableArrays(lu450GableLeftValue, lu450GableRightValue);
    ...
    if (true == lu450GableLeftValue) {
	    lineStyleB = { stroke: '#000', strokeWidth: '1px' };
    }
    
    if (true == lu450GableRightValue) {
  ...
  render3D: function (element, layer, scene) {
    ...
    if (cachedJSONLarderUnit450) {
      let loader = new ObjectLoader();
      let object = loader.parse(cachedJSONLarderUnit450);
      return Promise.resolve(onLoadItem(object));
    }

    return loadObjWithMaterial(mtl, obj, path.dirname(img1) + '/')
      .then(object => {
        cachedJSONLarderUnit450 = object.toJSON();
        let loader = new ObjectLoader();
        return onLoadItem(loader.parse(cachedJSONLarderUnit450))
      });
