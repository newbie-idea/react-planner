...
import {sceneValues, lu350GableArrays} from '../../utils/mtl-exports';

let globalTexture;
let worktopTexture;
let lu350GableLeftValue;
let lu350GableRightValue;
let kickboardValue;
let hingeNumber;

const mtl = require('./larder-unit-350.mtl');
const obj = require('./larder-unit-350.obj');
...
const width = {length: 350, unit: 'mm'};
const depth = {length: 590, unit: 'mm'};
const height = {length: 2120, unit: 'mm'};

let cachedJSONLarderUnit350 = null;

export default {
  name: 'Larder Unit 350',
  prototype: 'items',

  info: {
    title: 'LU 350',
    tag: ['single', 'base'],
    description: 'Larder Unit 350',
    image: require('../../images/LU-350.png')
  },
  ...
  render2D: function (element, layer, scene) {
    ...
    lu350GableLeftValue = element.properties.get('gableLeft');
    lu350GableRightValue = element.properties.get('gableRight');
    lu350GableArrays(lu350GableLeftValue, lu350GableRightValue);
    ...
    if (true == lu350GableLeftValue) {
	    lineStyleB = { stroke: '#000', strokeWidth: '1px' };
    }
    
    if (true == lu350GableRightValue) {
  ...
  render3D: function (element, layer, scene) {
    ...
    if (cachedJSONLarderUnit350) {
      let loader = new ObjectLoader();
      let object = loader.parse(cachedJSONLarderUnit350);
      return Promise.resolve(onLoadItem(object));
    }

    return loadObjWithMaterial(mtl, obj, path.dirname(img1) + '/')
      .then(object => {
        cachedJSONLarderUnit350 = object.toJSON();
        let loader = new ObjectLoader();
        return onLoadItem(loader.parse(cachedJSONLarderUnit350))
      });
