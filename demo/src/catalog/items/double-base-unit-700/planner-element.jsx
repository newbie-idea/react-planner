...
import {sceneValues, dbu700GableArrays} from '../../utils/mtl-exports';

let globalTexture;
let worktopTexture;
let dbu700GableLeftValue;
let dbu700GableRightValue;
let kickboardValue;
let hingeNumber;

const mtl = require('./double-base-unit-700.mtl');
const obj = require('./double-base-unit-700.obj');
...
let cachedJSONDoubleBaseUnit700 = null;

export default {
  name: 'Double Base Unit 700',
  prototype: 'items',

  info: {
    title: 'DBU 700',
    tag: ['double', 'base'],
    description: 'Double Base Unit 700',
    image: require('../../images/DBU-700.png')
  },
  ...
  render2D: function (element, layer, scene) {
    ...
    dbu700GableLeftValue = element.properties.get('gableLeft');
    dbu700GableRightValue = element.properties.get('gableRight');
    dbu700GableArrays(dbu700GableLeftValue, dbu700GableRightValue);
    ...
    if (true == dbu700GableLeftValue) {
	    lineStyleB = { stroke: '#000', strokeWidth: '1px' };
    }
    
    if (true == dbu700GableRightValue) {
  ...
  render3D: function (element, layer, scene) {
    ...
    if (cachedJSONDoubleBaseUnit700) {
      let loader = new ObjectLoader();
      let object = loader.parse(cachedJSONDoubleBaseUnit700);
      return Promise.resolve(onLoadItem(object));
    }

    return loadObjWithMaterial(mtl, obj, path.dirname(img1) + '/')
      .then(object => {
        cachedJSONDoubleBaseUnit700 = object.toJSON();
        let loader = new ObjectLoader();
        return onLoadItem(loader.parse(cachedJSONDoubleBaseUnit700))
      });
