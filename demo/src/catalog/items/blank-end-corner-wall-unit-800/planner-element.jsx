...
import {sceneValues, becwu800GableArrays} from '../../utils/mtl-exports';

let globalTexture;
let worktopTexture;
let becwu800GableLeftValue;
let becwu800GableRightValue;
let pelmetValue;
let corniceValue;
let hingeNumber;
let doorNumber;

const mtl = require('./double-wall-unit-800.mtl');
const obj = require('./double-wall-unit-800.obj');
...
let cachedJSONBlankEndCornerWallUnit800 = null;

export default {
  name: 'Blank End Corner Wall Unit 800',
  prototype: 'items',

  info: {
    title: 'BECWU 800',
    tag: ['blank end', 'wall'],
    description: 'Blank End Corner Wall Unit 800',
    image: require('../../images/BECWU-800.png')
  },
  ...
  render2D: function (element, layer, scene) {
    ...
    becwu800GableLeftValue = element.properties.get('gableLeft');
    becwu800GableRightValue = element.properties.get('gableRight');
    becwu800GableArrays(becwu800GableLeftValue, becwu800GableRightValue);
    ...
    if (true == becwu800GableLeftValue) {
	    lineStyleB = { stroke: '#000', strokeWidth: '1px' };
    }
    
    if (true == becwu800GableRightValue) {
  ...
  render3D: function (element, layer, scene) {
    ...
    if (cachedJSONBlankEndCornerWallUnit800) {
      let loader = new ObjectLoader();
      let object = loader.parse(cachedJSONBlankEndCornerWallUnit800);
      return Promise.resolve(onLoadItem(object));
    }

    return loadObjWithMaterial(mtl, obj, path.dirname(img1) + '/')
      .then(object => {
        cachedJSONBlankEndCornerWallUnit800 = object.toJSON();
        let loader = new ObjectLoader();
        return onLoadItem(loader.parse(cachedJSONBlankEndCornerWallUnit800))
      });
