...
import {sceneValues, swu400GableArrays} from '../../utils/mtl-exports';

let globalTexture;
let worktopTexture;
let swu400GableLeftValue;
let swu400GableRightValue;
let pelmetValue;
let corniceValue;
let hingeNumber;

const mtl = require('./single-wall-unit-400.mtl');
const obj = require('./single-wall-unit-400.obj');
...
let cachedJSONSingleWallUnit400 = null;

export default {
  name: 'Single Wall Unit 400',
  prototype: 'items',

  info: {
    title: 'SWU 400',
    tag: ['single', 'wall'],
    description: 'Single Wall Unit 400',
    image: require('../../images/SWU-400.png')
  },
  ...
  render2D: function (element, layer, scene) {
    ...
    swu400GableLeftValue = element.properties.get('gableLeft');
    swu400GableRightValue = element.properties.get('gableRight');
    swu400GableArrays(swu400GableLeftValue, swu400GableRightValue);
    ...
    if (true == swu400GableLeftValue) {
	    lineStyleB = { stroke: '#000', strokeWidth: '1px' };
    }
    
    if (true == swu400GableRightValue) {
  ...
  render3D: function (element, layer, scene) {
    ...
    if (cachedJSONSingleWallUnit400) {
      let loader = new ObjectLoader();
      let object = loader.parse(cachedJSONSingleWallUnit400);
      return Promise.resolve(onLoadItem(object));
    }

    return loadObjWithMaterial(mtl, obj, path.dirname(img1) + '/')
      .then(object => {
        cachedJSONSingleWallUnit400 = object.toJSON();
        let loader = new ObjectLoader();
        return onLoadItem(loader.parse(cachedJSONSingleWallUnit400))
      });
