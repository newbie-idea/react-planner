...
import {sceneValues, cbu1100GableArrays} from '../../utils/mtl-exports';

let globalTexture;
let worktopTexture;
let cbu1100GableLeftValue;
let cbu1100GableRightValue;
let kickboardValue;
let hingeNumber;

const mtl = require('./corner-base-unit-1100.mtl');
const obj = require('./corner-base-unit-1100.obj');
...
const depth = {length: 1100, unit: 'mm'};
const height = {length: 870, unit: 'mm'};

let cachedJSONCornerBaseUnit1100 = null;

export default {
  name: 'Corner Base Unit 1100',
  prototype: 'items',

  info: {
    title: 'CBU 1100',
    tag: ['corner', 'base'],
    description: 'Corner Base Unit 1100',
    image: require('../../images/CBU-1100.png')
  },
  ...
  render2D: function (element, layer, scene) {
    ...
    cbu1100GableLeftValue = element.properties.get('gableLeft');
    cbu1100GableRightValue = element.properties.get('gableRight');
    cbu1100GableArrays(cbu1100GableLeftValue, cbu1100GableRightValue);
    ...
    if (true == cbu1100GableLeftValue) {
	    lineStyleB = { stroke: '#000', strokeWidth: '1px' };
    }
    
    if (true == cbu1100GableRightValue) {
  ...
  render3D: function (element, layer, scene) {
    ...
    if (cachedJSONCornerBaseUnit1100) {
      let loader = new ObjectLoader();
      let object = loader.parse(cachedJSONCornerBaseUnit1100);
      return Promise.resolve(onLoadItem(object));
    }

    return loadObjWithMaterial(mtl, obj, path.dirname(img1) + '/')
      .then(object => {
        cachedJSONCornerBaseUnit1100 = object.toJSON();
        let loader = new ObjectLoader();
        return onLoadItem(loader.parse(cachedJSONCornerBaseUnit1100))
      });
