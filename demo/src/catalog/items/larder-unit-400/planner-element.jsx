...
import {sceneValues, lu400GableArrays} from '../../utils/mtl-exports';

let globalTexture;
let worktopTexture;
let lu400GableLeftValue;
let lu400GableRightValue;
let kickboardValue;
let hingeNumber;

const mtl = require('./larder-unit-400.mtl');
const obj = require('./larder-unit-400.obj');
...
const width = {length: 400, unit: 'mm'};
const depth = {length: 590, unit: 'mm'};
const height = {length: 2120, unit: 'mm'};

let cachedJSONLarderUnit400 = null;

export default {
  name: 'Larder Unit 400',
  prototype: 'items',

  info: {
    title: 'LU 400',
    tag: ['single', 'base'],
    description: 'Larder Unit 400',
    image: require('../../images/LU-400.png')
  },
  ...
  render2D: function (element, layer, scene) {
    ...
    lu400GableLeftValue = element.properties.get('gableLeft');
    lu400GableRightValue = element.properties.get('gableRight');
    lu400GableArrays(lu400GableLeftValue, lu400GableRightValue);
    ...
    if (true == lu400GableLeftValue) {
	    lineStyleB = { stroke: '#000', strokeWidth: '1px' };
    }
    
    if (true == lu400GableRightValue) {
  ...
  render3D: function (element, layer, scene) {
    ...
    if (cachedJSONLarderUnit400) {
      let loader = new ObjectLoader();
      let object = loader.parse(cachedJSONLarderUnit400);
      return Promise.resolve(onLoadItem(object));
    }

    return loadObjWithMaterial(mtl, obj, path.dirname(img1) + '/')
      .then(object => {
        cachedJSONLarderUnit400 = object.toJSON();
        let loader = new ObjectLoader();
        return onLoadItem(loader.parse(cachedJSONLarderUnit400))
      });
