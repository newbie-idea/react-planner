...
import {sceneValues, dp350GableArrays} from '../../utils/mtl-exports';

let globalTexture;
let worktopTexture;
let dp350GableLeftValue;
let dp350GableRightValue;
let kickboardValue;

const mtl = require('./drawer-pack-350.mtl');
const obj = require('./drawer-pack-350.obj');
...
let cachedJSONDrawerPack350 = null;

export default {
  name: 'Drawer Pack 350',
  prototype: 'items',

  info: {
    title: 'DP 350',
    tag: ['single', 'base'],
    description: 'Drawer Pack 350',
    image: require('../../images/DP-350.png')
  },
  ...
  render2D: function (element, layer, scene) {
    ...
    dp350GableLeftValue = element.properties.get('gableLeft');
    dp350GableRightValue = element.properties.get('gableRight');
    dp350GableArrays(dp350GableLeftValue, dp350GableRightValue);
    ...
    if (true == dp350GableLeftValue) {
	    lineStyleB = { stroke: '#000', strokeWidth: '1px' };
    }
    
    if (true == dp350GableRightValue) {
  ...
  render3D: function (element, layer, scene) {
    ...
    if (cachedJSONDrawerPack350) {
      let loader = new ObjectLoader();
      let object = loader.parse(cachedJSONDrawerPack350);
      return Promise.resolve(onLoadItem(object));
    }

    return loadObjWithMaterial(mtl, obj, path.dirname(img1) + '/')
      .then(object => {
        cachedJSONDrawerPack350 = object.toJSON();
        let loader = new ObjectLoader();
        return onLoadItem(loader.parse(cachedJSONDrawerPack350))
      });
