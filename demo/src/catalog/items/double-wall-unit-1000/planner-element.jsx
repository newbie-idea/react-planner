...
import {sceneValues, dwu1000GableArrays} from '../../utils/mtl-exports';

let globalTexture;
let worktopTexture;
let dwu1000GableLeftValue;
let dwu1000GableRightValue;
let pelmetValue;
let corniceValue;
let hingeNumber;

const mtl = require('./double-wall-unit-1000.mtl');
const obj = require('./double-wall-unit-1000.obj');
...
let cachedJSONDoubleWallUnit1000 = null;

export default {
  name: 'Double Wall Unit 1000',
  prototype: 'items',

  info: {
    title: 'DWU 1000',
    tag: ['double', 'wall'],
    description: 'Double Wall Unit 1000',
    image: require('../../images/DWU-1000.png')
  },
  ...
  render2D: function (element, layer, scene) {
    ...
    dwu1000GableLeftValue = element.properties.get('gableLeft');
    dwu1000GableRightValue = element.properties.get('gableRight');
    dwu1000GableArrays(dwu1000GableLeftValue, dwu1000GableRightValue);
    ...
    if (true == dwu1000GableLeftValue) {
	    lineStyleB = { stroke: '#000', strokeWidth: '1px' };
    }
    
    if (true == dwu1000GableRightValue) {
  ...
  render3D: function (element, layer, scene) {
    ...
    if (cachedJSONDoubleWallUnit1000) {
      let loader = new ObjectLoader();
      let object = loader.parse(cachedJSONDoubleWallUnit1000);
      return Promise.resolve(onLoadItem(object));
    }

    return loadObjWithMaterial(mtl, obj, path.dirname(img1) + '/')
      .then(object => {
        cachedJSONDoubleWallUnit1000 = object.toJSON();
        let loader = new ObjectLoader();
        return onLoadItem(loader.parse(cachedJSONDoubleWallUnit1000))
      });
