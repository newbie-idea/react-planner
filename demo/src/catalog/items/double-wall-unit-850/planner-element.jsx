...
import {sceneValues, dwu850GableArrays} from '../../utils/mtl-exports';

let globalTexture;
let worktopTexture;
let dwu850GableLeftValue;
let dwu850GableRightValue;
let pelmetValue;
let corniceValue;
let hingeNumber;
...
let cachedJSONDoubleWallUnit850 = null;

export default {
  name: 'Double Wall Unit 850',
  prototype: 'items',

  info: {
    title: 'DWU 850',
    tag: ['double', 'wall'],
    description: 'Double Wall Unit 850',
    image: require('../../images/DWU-850.png')
  },
  ...
  render2D: function (element, layer, scene) {
    ...
    dwu850GableLeftValue = element.properties.get('gableLeft');
    dwu850GableRightValue = element.properties.get('gableRight');
    dwu850GableArrays(dwu850GableLeftValue, dwu850GableRightValue);
    ...
    if (true == dwu850GableLeftValue) {
	    lineStyleB = { stroke: '#000', strokeWidth: '1px' };
    }
    
    if (true == dwu850GableRightValue) {
  ...
  render3D: function (element, layer, scene) {
    ...
    if (cachedJSONDoubleWallUnit850) {
      let loader = new ObjectLoader();
      let object = loader.parse(cachedJSONDoubleWallUnit850);
      return Promise.resolve(onLoadItem(object));
    }

    return loadObjWithMaterial(mtl, obj, path.dirname(img1) + '/')
      .then(object => {
        cachedJSONDoubleWallUnit850 = object.toJSON();
        let loader = new ObjectLoader();
        return onLoadItem(loader.parse(cachedJSONDoubleWallUnit850))
      });
