...
import {sceneValues, cbu800GableArrays} from '../../utils/mtl-exports';

let globalTexture;
let worktopTexture;
let cbu800GableLeftValue;
let cbu800GableRightValue;
let kickboardValue;
let hingeNumber;

const mtl = require('./corner-base-unit-800.mtl');
const obj = require('./corner-base-unit-800.obj');
...
const depth = {length: 800, unit: 'mm'};
const height = {length: 870, unit: 'mm'};

let cachedJSONCornerBaseUnit800 = null;

export default {
  name: 'Corner Base Unit 800',
  prototype: 'items',

  info: {
    title: 'CBU 800',
    tag: ['corner', 'base'],
    description: 'Corner Base Unit 800',
    image: require('../../images/CBU-800.png')
  },
  ...
  render2D: function (element, layer, scene) {
    ...
    cbu800GableLeftValue = element.properties.get('gableLeft');
    cbu800GableRightValue = element.properties.get('gableRight');
    cbu800GableArrays(cbu800GableLeftValue, cbu800GableRightValue);
    ...
    if (true == cbu800GableLeftValue) {
	    lineStyleB = { stroke: '#000', strokeWidth: '1px' };
    }
    
    if (true == cbu800GableRightValue) {
  ...
  render3D: function (element, layer, scene) {
    ...
    if (cachedJSONCornerBaseUnit800) {
      let loader = new ObjectLoader();
      let object = loader.parse(cachedJSONCornerBaseUnit800);
      return Promise.resolve(onLoadItem(object));
    }

    return loadObjWithMaterial(mtl, obj, path.dirname(img1) + '/')
      .then(object => {
        cachedJSONCornerBaseUnit800 = object.toJSON();
        let loader = new ObjectLoader();
        return onLoadItem(loader.parse(cachedJSONCornerBaseUnit800))
      });
