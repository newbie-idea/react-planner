...
import {sceneValues, sbu550GableArrays} from '../../utils/mtl-exports';

let globalTexture;
let worktopTexture;
let sbu550GableLeftValue;
let sbu550GableRightValue;
let kickboardValue;
let hingeNumber;

const mtl = require('./single-base-unit-550.mtl');
const obj = require('./single-base-unit-550.obj');
...
let cachedJSONSingleBaseUnit550 = null;

export default {
  name: 'Single Base Unit 550',
  prototype: 'items',

  info: {
    title: 'SBU 550',
    tag: ['single', 'base'],
    description: 'Single Base Unit 550',
    image: require('../../images/SBU-550.png')
  },
  ...
  render2D: function (element, layer, scene) {
    ...
    sbu550GableLeftValue = element.properties.get('gableLeft');
    sbu550GableRightValue = element.properties.get('gableRight');
    sbu550GableArrays(sbu550GableLeftValue, sbu550GableRightValue);
    ...
    if (true == sbu550GableLeftValue) {
	    lineStyleB = { stroke: '#000', strokeWidth: '1px' };
    }
    
    if (true == sbu550GableRightValue) {
  ...
  render3D: function (element, layer, scene) {
    ...
    if (cachedJSONSingleBaseUnit550) {
      let loader = new ObjectLoader();
      let object = loader.parse(cachedJSONSingleBaseUnit550);
      return Promise.resolve(onLoadItem(object));
    }

    return loadObjWithMaterial(mtl, obj, path.dirname(img1) + '/')
      .then(object => {
        cachedJSONSingleBaseUnit550 = object.toJSON();
        let loader = new ObjectLoader();
        return onLoadItem(loader.parse(cachedJSONSingleBaseUnit550))
      });
