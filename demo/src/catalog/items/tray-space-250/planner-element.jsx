...
import {sceneValues, ts250GableArrays} from '../../utils/mtl-exports';

let globalTexture;
let worktopTexture;
let ts250GableLeftValue;
let ts250GableRightValue;
let kickboardValue;
let hingeNumber;

const mtl = require('./tray-space-250.mtl');
const obj = require('./tray-space-250.obj');
...
let cachedJSONTraySpace250 = null;
...
export default {
  ...
  info: {
    title: 'TS 250',
    tag: ['single', 'base'],
    description: 'Tray Space 250',
    image: require('../../images/TS-250.png')
  },
  ...
  render2D: function (element, layer, scene) {
    ...
    ts250GableLeftValue = element.properties.get('gableLeft');
    ts250GableRightValue = element.properties.get('gableRight');
    ts250GableArrays(ts250GableLeftValue, ts250GableRightValue);
    ...
    if (true == ts250GableLeftValue) {
	    lineStyleB = { stroke: '#000', strokeWidth: '1px' };
    }
    
    if (true == ts250GableRightValue) {
  ...
  render3D: function (element, layer, scene) {
    ...
    if (cachedJSONTraySpace250) {
      let loader = new ObjectLoader();
      let object = loader.parse(cachedJSONTraySpace250);
      return Promise.resolve(onLoadItem(object));
    }

    return loadObjWithMaterial(mtl, obj, path.dirname(img1) + '/')
      .then(object => {
        cachedJSONTraySpace250 = object.toJSON();
        let loader = new ObjectLoader();
        return onLoadItem(loader.parse(cachedJSONTraySpace250))
      });
