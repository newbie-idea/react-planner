...
import {sceneValues, ts150GableArrays} from '../../utils/mtl-exports';

let globalTexture;
let worktopTexture;
let ts150GableLeftValue;
let ts150GableRightValue;
let kickboardValue;
let hingeNumber;

const mtl = require('./tray-space-150.mtl');
const obj = require('./tray-space-150.obj');
...
const width = {length: 150, unit: 'mm'};
const depth = {length: 590, unit: 'mm'};
const height = {length: 870, unit: 'mm'};

let cachedJSONTraySpace150 = null;
...
export default {
  ...
  info: {
    title: 'TS 150',
    tag: ['single', 'base'],
    description: 'Tray Space 150',
    image: require('../../images/TS-150.png')
  },
  ...
  render2D: function (element, layer, scene) {
    ...
    ts150GableLeftValue = element.properties.get('gableLeft');
    ts150GableRightValue = element.properties.get('gableRight');
    ts150GableArrays(ts150GableLeftValue, ts150GableRightValue);
    ...
    if (true == ts150GableLeftValue) {
	    lineStyleB = { stroke: '#000', strokeWidth: '1px' };
    }
    
    if (true == ts150GableRightValue) {
  ...
  render3D: function (element, layer, scene) {
    ...
    if (cachedJSONTraySpace150) {
      let loader = new ObjectLoader();
      let object = loader.parse(cachedJSONTraySpace150);
      return Promise.resolve(onLoadItem(object));
    }

    return loadObjWithMaterial(mtl, obj, path.dirname(img1) + '/')
      .then(object => {
        cachedJSONTraySpace150 = object.toJSON();
        let loader = new ObjectLoader();
        return onLoadItem(loader.parse(cachedJSONTraySpace150))
      });
