...
import {sceneValues, dp550GableArrays} from '../../utils/mtl-exports';

let globalTexture;
let worktopTexture;
let dp550GableLeftValue;
let dp550GableRightValue;
let kickboardValue;

const mtl = require('./drawer-pack-550.mtl');
const obj = require('./drawer-pack-550.obj');
...
let cachedJSONDrawerPack550 = null;

export default {
  name: 'Drawer Pack 550',
  prototype: 'items',

  info: {
    title: 'DP 550',
    tag: ['single', 'base'],
    description: 'Drawer Pack 550',
    image: require('../../images/DP-550.png')
  },
  ...
  render2D: function (element, layer, scene) {
    ...
    dp550GableLeftValue = element.properties.get('gableLeft');
    dp550GableRightValue = element.properties.get('gableRight');
    dp550GableArrays(dp550GableLeftValue, dp550GableRightValue);
    ...
    if (true == dp550GableLeftValue) {
	    lineStyleB = { stroke: '#000', strokeWidth: '1px' };
    }
    
    if (true == dp550GableRightValue) {
  ...
  render3D: function (element, layer, scene) {
    ...
    if (cachedJSONDrawerPack550) {
      let loader = new ObjectLoader();
      let object = loader.parse(cachedJSONDrawerPack550);
      return Promise.resolve(onLoadItem(object));
    }

    return loadObjWithMaterial(mtl, obj, path.dirname(img1) + '/')
      .then(object => {
        cachedJSONDrawerPack550 = object.toJSON();
        let loader = new ObjectLoader();
        return onLoadItem(loader.parse(cachedJSONDrawerPack550))
      });
