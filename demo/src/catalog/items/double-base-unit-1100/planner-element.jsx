...
import {sceneValues, dbu1100GableArrays} from '../../utils/mtl-exports';

let globalTexture;
let worktopTexture;
let dbu1100GableLeftValue;
let dbu1100GableRightValue;
let kickboardValue;
let hingeNumber;
...
let cachedJSONDoubleBaseUnit1100 = null;

export default {
  name: 'Double Base Unit 1100',
  prototype: 'items',

  info: {
    title: 'DBU 1100',
    tag: ['double', 'base'],
    description: 'Double Base Unit 1100',
    image: require('../../images/DBU-1100.png')
  },
  ...
  render2D: function (element, layer, scene) {
    ...
    dbu1100GableLeftValue = element.properties.get('gableLeft');
    dbu1100GableRightValue = element.properties.get('gableRight');
    dbu1100GableArrays(dbu1100GableLeftValue, dbu1100GableRightValue);
    ...
    if (true == dbu1100GableLeftValue) {
	    lineStyleB = { stroke: '#000', strokeWidth: '1px' };
    }
    
    if (true == dbu1100GableRightValue) {
  ...
  render3D: function (element, layer, scene) {
    ...
    if (cachedJSONDoubleBaseUnit1100) {
      let loader = new ObjectLoader();
      let object = loader.parse(cachedJSONDoubleBaseUnit1100);
      return Promise.resolve(onLoadItem(object));
    }

    return loadObjWithMaterial(mtl, obj, path.dirname(img1) + '/')
      .then(object => {
        cachedJSONDoubleBaseUnit1100 = object.toJSON();
        let loader = new ObjectLoader();
        return onLoadItem(loader.parse(cachedJSONDoubleBaseUnit1100))
      });
