...
import {sceneValues, sbu250GableArrays} from '../../utils/mtl-exports';

let globalTexture;
let worktopTexture;
let sbu250GableLeftValue;
let sbu250GableRightValue;
let kickboardValue;
let hingeNumber;

const mtl = require('./single-base-unit-250.mtl');
const obj = require('./single-base-unit-250.obj');
...
const width = {length: 250, unit: 'mm'};
const depth = {length: 590, unit: 'mm'};
const height = {length: 870, unit: 'mm'};

let cachedJSONSingleBaseUnit250 = null;

export default {
  name: 'Single Base Unit 250',
  prototype: 'items',

  info: {
    title: 'SBU 250',
    tag: ['single', 'base'],
    description: 'Single Base Unit 250',
    image: require('../../images/SBU-250.png')
  },
  ...
  render2D: function (element, layer, scene) {
    ...
    sbu250GableLeftValue = element.properties.get('gableLeft');
    sbu250GableRightValue = element.properties.get('gableRight');
    sbu250GableArrays(sbu250GableLeftValue, sbu250GableRightValue);
    ...
    if (true == sbu250GableLeftValue) {
	    lineStyleB = { stroke: '#000', strokeWidth: '1px' };
    }
    
    if (true == sbu250GableRightValue) {
  ...
  render3D: function (element, layer, scene) {
    ...
    if (cachedJSONSingleBaseUnit250) {
      let loader = new ObjectLoader();
      let object = loader.parse(cachedJSONSingleBaseUnit250);
      return Promise.resolve(onLoadItem(object));
    }

    return loadObjWithMaterial(mtl, obj, path.dirname(img1) + '/')
      .then(object => {
        cachedJSONSingleBaseUnit250 = object.toJSON();
        let loader = new ObjectLoader();
        return onLoadItem(loader.parse(cachedJSONSingleBaseUnit250))
      });
