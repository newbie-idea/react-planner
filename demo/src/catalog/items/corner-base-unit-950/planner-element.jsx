...
import {sceneValues, cbu950GableArrays} from '../../utils/mtl-exports';

let globalTexture;
let worktopTexture;
let cbu950GableLeftValue;
let cbu950GableRightValue;
let kickboardValue;
let hingeNumber;

const mtl = require('./corner-base-unit-950.mtl');
const obj = require('./corner-base-unit-950.obj');
...
const depth = {length: 950, unit: 'mm'};
const height = {length: 870, unit: 'mm'};

let cachedJSONCornerBaseUnit950 = null;

export default {
  name: 'Corner Base Unit 950',
  prototype: 'items',

  info: {
    title: 'CBU 950',
    tag: ['corner', 'base'],
    description: 'Corner Base Unit 950',
    image: require('../../images/CBU-950.png')
  },
  ...
  render2D: function (element, layer, scene) {
    ...
    cbu950GableLeftValue = element.properties.get('gableLeft');
    cbu950GableRightValue = element.properties.get('gableRight');
    cbu950GableArrays(cbu950GableLeftValue, cbu950GableRightValue);
    ...
    if (true == cbu950GableLeftValue) {
	    lineStyleB = { stroke: '#000', strokeWidth: '1px' };
    }
    
    if (true == cbu950GableRightValue) {
  ...
  render3D: function (element, layer, scene) {
    ...
    if (cachedJSONCornerBaseUnit950) {
      let loader = new ObjectLoader();
      let object = loader.parse(cachedJSONCornerBaseUnit950);
      return Promise.resolve(onLoadItem(object));
    }

    return loadObjWithMaterial(mtl, obj, path.dirname(img1) + '/')
      .then(object => {
        cachedJSONCornerBaseUnit950 = object.toJSON();
        let loader = new ObjectLoader();
        return onLoadItem(loader.parse(cachedJSONCornerBaseUnit950))
      });
