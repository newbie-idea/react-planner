...
import {sceneValues, cbu1000GableArrays} from '../../utils/mtl-exports';

let globalTexture;
let worktopTexture;
let cbu1000GableLeftValue;
let cbu1000GableRightValue;
let kickboardValue;
let hingeNumber;

const mtl = require('./corner-base-unit-1000.mtl');
const obj = require('./corner-base-unit-1000.obj');
...
const depth = {length: 1000, unit: 'mm'};
const height = {length: 870, unit: 'mm'};

let cachedJSONCornerBaseUnit1000 = null;

export default {
  name: 'Corner Base Unit 1000',
  prototype: 'items',

  info: {
    title: 'CBU 1000',
    tag: ['corner', 'base'],
    description: 'Corner Base Unit 1000',
    image: require('../../images/CBU-1000.png')
  },
  ...
  render2D: function (element, layer, scene) {
    ...
    cbu1000GableLeftValue = element.properties.get('gableLeft');
    cbu1000GableRightValue = element.properties.get('gableRight');
    cbu1000GableArrays(cbu1000GableLeftValue, cbu1000GableRightValue);
    ...
    if (true == cbu1000GableLeftValue) {
	    lineStyleB = { stroke: '#000', strokeWidth: '1px' };
    }
    
    if (true == cbu1000GableRightValue) {
  ...
  render3D: function (element, layer, scene) {
    ...
    if (cachedJSONCornerBaseUnit1000) {
      let loader = new ObjectLoader();
      let object = loader.parse(cachedJSONCornerBaseUnit1000);
      return Promise.resolve(onLoadItem(object));
    }

    return loadObjWithMaterial(mtl, obj, path.dirname(img1) + '/')
      .then(object => {
        cachedJSONCornerBaseUnit1000 = object.toJSON();
        let loader = new ObjectLoader();
        return onLoadItem(loader.parse(cachedJSONCornerBaseUnit1000))
      });
