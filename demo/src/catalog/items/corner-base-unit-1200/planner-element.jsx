...
import {sceneValues, cbu1200GableArrays} from '../../utils/mtl-exports';

let globalTexture;
let worktopTexture;
let cbu1200GableLeftValue;
let cbu1200GableRightValue;
let kickboardValue;
let hingeNumber;

const mtl = require('./corner-base-unit-1200.mtl');
const obj = require('./corner-base-unit-1200.obj');
...
const depth = {length: 1200, unit: 'mm'};
const height = {length: 870, unit: 'mm'};

let cachedJSONCornerBaseUnit1200 = null;

export default {
  name: 'Corner Base Unit 1200',
  prototype: 'items',

  info: {
    title: 'CBU 1200',
    tag: ['corner', 'base'],
    description: 'Corner Base Unit 1200',
    image: require('../../images/CBU-1200.png')
  },
  ...
  render2D: function (element, layer, scene) {
    ...
    cbu1200GableLeftValue = element.properties.get('gableLeft');
    cbu1200GableRightValue = element.properties.get('gableRight');
    cbu1200GableArrays(cbu1200GableLeftValue, cbu1200GableRightValue);
    ...
    if (true == cbu1200GableLeftValue) {
	    lineStyleB = { stroke: '#000', strokeWidth: '1px' };
    }
    
    if (true == cbu1200GableRightValue) {
  ...
  render3D: function (element, layer, scene) {
    ...
    if (cachedJSONCornerBaseUnit1200) {
      let loader = new ObjectLoader();
      let object = loader.parse(cachedJSONCornerBaseUnit1200);
      return Promise.resolve(onLoadItem(object));
    }

    return loadObjWithMaterial(mtl, obj, path.dirname(img1) + '/')
      .then(object => {
        cachedJSONCornerBaseUnit1200 = object.toJSON();
        let loader = new ObjectLoader();
        return onLoadItem(loader.parse(cachedJSONCornerBaseUnit1200))
      });
