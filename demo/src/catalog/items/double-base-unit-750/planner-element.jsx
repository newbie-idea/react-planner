...
import {sceneValues, dbu750GableArrays} from '../../utils/mtl-exports';

let globalTexture;
let worktopTexture;
let dbu750GableLeftValue;
let dbu750GableRightValue;
let kickboardValue;
let hingeNumber;

const mtl = require('./double-base-unit-750.mtl');
const obj = require('./double-base-unit-750.obj');
...
let cachedJSONDoubleBaseUnit750 = null;

export default {
  name: 'Double Base Unit 750',
  prototype: 'items',

  info: {
    title: 'DBU 750',
    tag: ['double', 'base'],
    description: 'Double Base Unit 750',
    image: require('../../images/DBU-750.png')
  },
  ...
  render2D: function (element, layer, scene) {
    ...
    dbu750GableLeftValue = element.properties.get('gableLeft');
    dbu750GableRightValue = element.properties.get('gableRight');
    dbu750GableArrays(dbu750GableLeftValue, dbu750GableRightValue);
    ...
    if (true == dbu750GableLeftValue) {
	    lineStyleB = { stroke: '#000', strokeWidth: '1px' };
    }
    
    if (true == dbu750GableRightValue) {
  ...
  render3D: function (element, layer, scene) {
    ...
    if (cachedJSONDoubleBaseUnit750) {
      let loader = new ObjectLoader();
      let object = loader.parse(cachedJSONDoubleBaseUnit750);
      return Promise.resolve(onLoadItem(object));
    }

    return loadObjWithMaterial(mtl, obj, path.dirname(img1) + '/')
      .then(object => {
        cachedJSONDoubleBaseUnit750 = object.toJSON();
        let loader = new ObjectLoader();
        return onLoadItem(loader.parse(cachedJSONDoubleBaseUnit750))
      });
