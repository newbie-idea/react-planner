...
import {sceneValues, dbu650GableArrays} from '../../utils/mtl-exports';

let globalTexture;
let worktopTexture;
let dbu650GableLeftValue;
let dbu650GableRightValue;
let kickboardValue;
let hingeNumber;

const mtl = require('./double-base-unit-650.mtl');
const obj = require('./double-base-unit-650.obj');
...
let cachedJSONDoubleBaseUnit650 = null;

export default {
  name: 'Double Base Unit 650',
  prototype: 'items',

  info: {
    title: 'DBU 650',
    tag: ['double', 'base'],
    description: 'Double Base Unit 650',
    image: require('../../images/DBU-650.png')
  },
  ...
  render2D: function (element, layer, scene) {
    ...
    dbu650GableLeftValue = element.properties.get('gableLeft');
    dbu650GableRightValue = element.properties.get('gableRight');
    dbu650GableArrays(dbu650GableLeftValue, dbu650GableRightValue);
    ...
    if (true == dbu650GableLeftValue) {
	    lineStyleB = { stroke: '#000', strokeWidth: '1px' };
    }
    
    if (true == dbu650GableRightValue) {
  ...
  render3D: function (element, layer, scene) {
    ...
    if (cachedJSONDoubleBaseUnit650) {
      let loader = new ObjectLoader();
      let object = loader.parse(cachedJSONDoubleBaseUnit650);
      return Promise.resolve(onLoadItem(object));
    }

    return loadObjWithMaterial(mtl, obj, path.dirname(img1) + '/')
      .then(object => {
        cachedJSONDoubleBaseUnit650 = object.toJSON();
        let loader = new ObjectLoader();
        return onLoadItem(loader.parse(cachedJSONDoubleBaseUnit650))
      });
