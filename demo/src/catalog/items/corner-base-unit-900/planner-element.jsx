...
import {sceneValues, cbu900GableArrays} from '../../utils/mtl-exports';

let globalTexture;
let worktopTexture;
let cbu900GableLeftValue;
let cbu900GableRightValue;
let kickboardValue;
let hingeNumber;

const mtl = require('./corner-base-unit-900.mtl');
const obj = require('./corner-base-unit-900.obj');
...
const depth = {length: 900, unit: 'mm'};
const height = {length: 870, unit: 'mm'};

let cachedJSONCornerBaseUnit900 = null;

export default {
  name: 'Corner Base Unit 900',
  prototype: 'items',

  info: {
    title: 'CBU 900',
    tag: ['corner', 'base'],
    description: 'Corner Base Unit 900',
    image: require('../../images/CBU-900.png')
  },
  ...
  render2D: function (element, layer, scene) {
    ...
    cbu900GableLeftValue = element.properties.get('gableLeft');
    cbu900GableRightValue = element.properties.get('gableRight');
    cbu900GableArrays(cbu900GableLeftValue, cbu900GableRightValue);
    ...
    if (true == cbu900GableLeftValue) {
	    lineStyleB = { stroke: '#000', strokeWidth: '1px' };
    }
    
    if (true == cbu900GableRightValue) {
  ...
  render3D: function (element, layer, scene) {
    ...
    if (cachedJSONCornerBaseUnit900) {
      let loader = new ObjectLoader();
      let object = loader.parse(cachedJSONCornerBaseUnit900);
      return Promise.resolve(onLoadItem(object));
    }

    return loadObjWithMaterial(mtl, obj, path.dirname(img1) + '/')
      .then(object => {
        cachedJSONCornerBaseUnit900 = object.toJSON();
        let loader = new ObjectLoader();
        return onLoadItem(loader.parse(cachedJSONCornerBaseUnit900))
      });
