...
import {sceneValues, swu600GableArrays} from '../../utils/mtl-exports';

let globalTexture;
let worktopTexture;
let swu600GableLeftValue;
let swu600GableRightValue;
let pelmetValue;
let corniceValue;
let hingeNumber;

const mtl = require('./single-wall-unit-600.mtl');
const obj = require('./single-wall-unit-600.obj');
...
let cachedJSONSingleWallUnit600 = null;

export default {
  name: 'Single Wall Unit 600',
  prototype: 'items',

  info: {
    title: 'SWU 600',
    tag: ['single', 'wall'],
    description: 'Single Wall Unit 600',
    image: require('../../images/SWU-600.png')
  },
  ...
  render2D: function (element, layer, scene) {
    ...
    swu600GableLeftValue = element.properties.get('gableLeft');
    swu600GableRightValue = element.properties.get('gableRight');
    swu600GableArrays(swu600GableLeftValue, swu600GableRightValue);
    ...
    if (true == swu600GableLeftValue) {
	    lineStyleB = { stroke: '#000', strokeWidth: '1px' };
    }
    
    if (true == swu600GableRightValue) {
  ...
  render3D: function (element, layer, scene) {
    ...
    if (cachedJSONSingleWallUnit600) {
      let loader = new ObjectLoader();
      let object = loader.parse(cachedJSONSingleWallUnit600);
      return Promise.resolve(onLoadItem(object));
    }

    return loadObjWithMaterial(mtl, obj, path.dirname(img1) + '/')
      .then(object => {
        cachedJSONSingleWallUnit600 = object.toJSON();
        let loader = new ObjectLoader();
        return onLoadItem(loader.parse(cachedJSONSingleWallUnit600))
      });
