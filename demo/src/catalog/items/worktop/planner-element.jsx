...
import {hinges, textOffsetBD, xOffset1, xOffset2, doorOffset1, doorOffset2, textOffset1, textOffset2} from '../../utils/obj-exports';
import {sceneValues} from '../../utils/mtl-exports';

let globalTexture;
let worktopTexture;

const mtl = require('./worktop.mtl');
const obj = require('./worktop.obj');
...
const depth = {length: 600, unit: 'mm'};
const height = {length: 40, unit: 'mm'};

let cachedJSONWorktop = null;
...
export default {
  ...
  render3D: function (element, layer, scene) {
    ...
    let onLoadItem = (object) => {
      ...
      object.position.y += newHeight / 10 + newAltitude - 5;
      object.position.z -= center[2];

      return object;
    };

    if (cachedJSONWorktop) {
      let loader = new ObjectLoader();
      let object = loader.parse(cachedJSONWorktop);
      return Promise.resolve(onLoadItem(object));
    }

    return loadObjWithMaterial(mtl, obj, path.dirname(img1) + '/')
      .then(object => {
        cachedJSONWorktop = object.toJSON();
        let loader = new ObjectLoader();
        return onLoadItem(loader.parse(cachedJSONWorktop))
      });
