...
import {sceneValues, swu450GableArrays} from '../../utils/mtl-exports';

let globalTexture;
let worktopTexture;
let swu450GableLeftValue;
let swu450GableRightValue;
let pelmetValue;
let corniceValue;
let hingeNumber;

const mtl = require('./single-wall-unit-450.mtl');
const obj = require('./single-wall-unit-450.obj');
...
let cachedJSONSingleWallUnit450 = null;

export default {
  name: 'Single Wall Unit 450',
  prototype: 'items',

  info: {
    title: 'SWU 450',
    tag: ['single'],
    description: 'Single Wall Unit 450',
    image: require('../../images/SWU-450.png')
  },
  ...
  render2D: function (element, layer, scene) {
    ...
    swu450GableLeftValue = element.properties.get('gableLeft');
    swu450GableRightValue = element.properties.get('gableRight');
    swu450GableArrays(swu450GableLeftValue, swu450GableRightValue);
    ...
    if (true == swu450GableLeftValue) {
	    lineStyleB = { stroke: '#000', strokeWidth: '1px' };
    }
    
    if (true == swu450GableRightValue) {
  ...
  render3D: function (element, layer, scene) {
    ...
    if (cachedJSONSingleWallUnit450) {
      let loader = new ObjectLoader();
      let object = loader.parse(cachedJSONSingleWallUnit450);
      return Promise.resolve(onLoadItem(object));
    }

    return loadObjWithMaterial(mtl, obj, path.dirname(img1) + '/')
      .then(object => {
        cachedJSONSingleWallUnit450 = object.toJSON();
        let loader = new ObjectLoader();
        return onLoadItem(loader.parse(cachedJSONSingleWallUnit450))
      });
