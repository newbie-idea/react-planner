...
import {sceneValues, dwu1150GableArrays} from '../../utils/mtl-exports';

let globalTexture;
let worktopTexture;
let dwu1150GableLeftValue;
let dwu1150GableRightValue;
let pelmetValue;
let corniceValue;
let hingeNumber;

const mtl = require('./double-wall-unit-1150.mtl');
const obj = require('./double-wall-unit-1150.obj');
...
let cachedJSONDoubleWallUnit1150 = null;

export default {
  name: 'Double Wall Unit 1150',
  prototype: 'items',

  info: {
    title: 'DWU 1150',
    tag: ['double', 'wall'],
    description: 'Double Wall Unit 1150',
    image: require('../../images/DWU-1150.png')
  },
  ...
  render2D: function (element, layer, scene) {
    ...
    dwu1150GableLeftValue = element.properties.get('gableLeft');
    dwu1150GableRightValue = element.properties.get('gableRight');
    dwu1150GableArrays(dwu1150GableLeftValue, dwu1150GableRightValue);
    ...
    if (true == dwu1150GableLeftValue) {
	    lineStyleB = { stroke: '#000', strokeWidth: '1px' };
    }
    
    if (true == dwu1150GableRightValue) {
  ...
  render3D: function (element, layer, scene) {
    ...
    if (cachedJSONDoubleWallUnit1150) {
      let loader = new ObjectLoader();
      let object = loader.parse(cachedJSONDoubleWallUnit1150);
      return Promise.resolve(onLoadItem(object));
    }

    return loadObjWithMaterial(mtl, obj, path.dirname(img1) + '/')
      .then(object => {
        cachedJSONDoubleWallUnit1150 = object.toJSON();
        let loader = new ObjectLoader();
        return onLoadItem(loader.parse(cachedJSONDoubleWallUnit1150))
      });
