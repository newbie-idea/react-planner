...
import {sceneValues, sbu200GableArrays} from '../../utils/mtl-exports';

let globalTexture;
let worktopTexture;
let sbu200GableLeftValue;
let sbu200GableRightValue;
let kickboardValue;
let hingeNumber;

const mtl = require('./single-base-unit-200.mtl');
const obj = require('./single-base-unit-200.obj');
...
const width = {length: 200, unit: 'mm'};
const depth = {length: 590, unit: 'mm'};
const height = {length: 870, unit: 'mm'};

let cachedJSONSingleBaseUnit200 = null;

export default {
  name: 'Single Base Unit 200',
  prototype: 'items',

  info: {
    title: 'SBU 200',
    tag: ['single', 'base'],
    description: 'Single Base Unit 200',
    image: require('../../images/SBU-200.png')
  },
  ...
  render2D: function (element, layer, scene) {
    ...
    sbu200GableLeftValue = element.properties.get('gableLeft');
    sbu200GableRightValue = element.properties.get('gableRight');
    sbu200GableArrays(sbu200GableLeftValue, sbu200GableRightValue);
    ...
    let rotatedTextOffset = 2;

    let textRotation2 = 90;
    if (Math.sin(angle2 * Math.PI / 180) < 0) {
      textRotation2 = 270;
    ...
    if (true == sbu200GableLeftValue) {
	    lineStyleB = { stroke: '#000', strokeWidth: '1px' };
    }
    
    if (true == sbu200GableRightValue) {
      ...
      <g transform={`translate(${-newWidth / 1},${-newDepth / 1}) scale(2,2)`}>
        ...
        <line
          ...
          style={kickboardLineStyle}
		    />
        <text key='2' x='0' y='0'
          transform={`translate(${newWidth / 2 - rotatedTextOffset}, ${newDepth / 4}) scale(0.5,-0.5) rotate(${textRotation2})`}
  ...
  render3D: function (element, layer, scene) {
    ...
    if (cachedJSONSingleBaseUnit200) {
      let loader = new ObjectLoader();
      let object = loader.parse(cachedJSONSingleBaseUnit200);
      return Promise.resolve(onLoadItem(object));
    }

    return loadObjWithMaterial(mtl, obj, path.dirname(img1) + '/')
      .then(object => {
        cachedJSONSingleBaseUnit200 = object.toJSON();
        let loader = new ObjectLoader();
        return onLoadItem(loader.parse(cachedJSONSingleBaseUnit200))
      });
