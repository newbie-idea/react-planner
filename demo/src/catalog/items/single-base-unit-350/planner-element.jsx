...
import {sceneValues, sbu350GableArrays} from '../../utils/mtl-exports';

let globalTexture;
let worktopTexture;
let sbu350GableLeftValue;
let sbu350GableRightValue;
let kickboardValue;
let hingeNumber;

const mtl = require('./single-base-unit-350.mtl');
const obj = require('./single-base-unit-350.obj');
...
let cachedJSONSingleBaseUnit350 = null;

export default {
  name: 'Single Base Unit 350',
  prototype: 'items',

  info: {
    title: 'SBU 350',
    tag: ['single', 'base'],
    description: 'Single Base Unit 350',
    image: require('../../images/SBU-350.png')
  },
  ...
  render2D: function (element, layer, scene) {
    ...
    sbu350GableLeftValue = element.properties.get('gableLeft');
    sbu350GableRightValue = element.properties.get('gableRight');
    sbu350GableArrays(sbu350GableLeftValue, sbu350GableRightValue);
    ...
    if (true == sbu350GableLeftValue) {
	    lineStyleB = { stroke: '#000', strokeWidth: '1px' };
    }
    
    if (true == sbu350GableRightValue) {
  ...
  render3D: function (element, layer, scene) {
    ...
    if (cachedJSONSingleBaseUnit350) {
      let loader = new ObjectLoader();
      let object = loader.parse(cachedJSONSingleBaseUnit350);
      return Promise.resolve(onLoadItem(object));
    }

    return loadObjWithMaterial(mtl, obj, path.dirname(img1) + '/')
      .then(object => {
        cachedJSONSingleBaseUnit350 = object.toJSON();
        let loader = new ObjectLoader();
        return onLoadItem(loader.parse(cachedJSONSingleBaseUnit350))
      });
