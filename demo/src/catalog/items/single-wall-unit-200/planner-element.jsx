...
import {sceneValues, swu200GableArrays} from '../../utils/mtl-exports';

let globalTexture;
let worktopTexture;
let swu200GableLeftValue;
let swu200GableRightValue;
let pelmetValue;
let corniceValue;
let hingeNumber;

const mtl = require('./single-wall-unit-200.mtl');
const obj = require('./single-wall-unit-200.obj');
...
let cachedJSONSingleWallUnit200 = null;

export default {
  name: 'Single Wall Unit 200',
  prototype: 'items',

  info: {
    title: 'SWU 200',
    tag: ['single', 'wall'],
    description: 'Single Wall Unit 200',
    image: require('../../images/SWU-200.png')
  },
  ...
  render2D: function (element, layer, scene) {
    ...
    swu200GableLeftValue = element.properties.get('gableLeft');
    swu200GableRightValue = element.properties.get('gableRight');
    swu200GableArrays(swu200GableLeftValue, swu200GableRightValue);
    ...
    if (true == swu200GableLeftValue) {
	    lineStyleB = { stroke: '#000', strokeWidth: '1px' };
    }
    
    if (true == swu200GableRightValue) {
      ...
      <g transform={`translate(${-newWidth / 1},${-newDepth / 1}) scale(2,2)`}>
        ...
        <line
          ...
          style={corniceLineStyle}
		    />
        <text key='2' x='0' y='0'
          transform={`translate(${newWidth / 2 - rotatedTextOffset}, ${newDepth / 2}) scale(0.5,-0.5) rotate(${textRotation2})`}
  ...
  render3D: function (element, layer, scene) {
    ...
    if (cachedJSONSingleWallUnit200) {
      let loader = new ObjectLoader();
      let object = loader.parse(cachedJSONSingleWallUnit200);
      return Promise.resolve(onLoadItem(object));
    }

    return loadObjWithMaterial(mtl, obj, path.dirname(img1) + '/')
      .then(object => {
        cachedJSONSingleWallUnit200 = object.toJSON();
        let loader = new ObjectLoader();
        return onLoadItem(loader.parse(cachedJSONSingleWallUnit200))
      });
