...
import {sceneValues, lu550GableArrays} from '../../utils/mtl-exports';

let globalTexture;
let worktopTexture;
let lu550GableLeftValue;
let lu550GableRightValue;
let kickboardValue;
let hingeNumber;

const mtl = require('./larder-unit-550.mtl');
const obj = require('./larder-unit-550.obj');
...
const width = {length: 550, unit: 'mm'};
const depth = {length: 590, unit: 'mm'};
const height = {length: 2120, unit: 'mm'};

let cachedJSONLarderUnit550 = null;

export default {
  name: 'Larder Unit 550',
  prototype: 'items',

  info: {
    title: 'LU 550',
    tag: ['single', 'base'],
    description: 'Larder Unit 550',
    image: require('../../images/LU-550.png')
  },
  ...
  render2D: function (element, layer, scene) {
    ...
    lu550GableLeftValue = element.properties.get('gableLeft');
    lu550GableRightValue = element.properties.get('gableRight');
    lu550GableArrays(lu550GableLeftValue, lu550GableRightValue);
    ...
    if (true == lu550GableLeftValue) {
	    lineStyleB = { stroke: '#000', strokeWidth: '1px' };
    }
    
    if (true == lu550GableRightValue) {
  ...
  render3D: function (element, layer, scene) {
    ...
    let onLoadItem = (object) => {
      ...
      object.position.y += newHeight / 10 + newAltitude - 213;
      object.position.z -= center[2];

      return object;
    };

    if (cachedJSONLarderUnit550) {
      let loader = new ObjectLoader();
      let object = loader.parse(cachedJSONLarderUnit550);
      return Promise.resolve(onLoadItem(object));
    }

    return loadObjWithMaterial(mtl, obj, path.dirname(img1) + '/')
      .then(object => {
        cachedJSONLarderUnit550 = object.toJSON();
        let loader = new ObjectLoader();
        return onLoadItem(loader.parse(cachedJSONLarderUnit550))
      });
