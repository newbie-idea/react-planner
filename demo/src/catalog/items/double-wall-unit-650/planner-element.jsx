...
import {sceneValues, dwu650GableArrays} from '../../utils/mtl-exports';

let globalTexture;
let worktopTexture;
let dwu650GableLeftValue;
let dwu650GableRightValue;
let pelmetValue;
let corniceValue;
let hingeNumber;
...
let cachedJSONDoubleWallUnit650 = null;

export default {
  name: 'Double Wall Unit 650',
  prototype: 'items',

  info: {
    title: 'DWU 650',
    tag: ['double', 'wall'],
    description: 'Double Wall Unit 650',
    image: require('../../images/DWU-650.png')
  },
  ...
  render2D: function (element, layer, scene) {
    ...
    dwu650GableLeftValue = element.properties.get('gableLeft');
    dwu650GableRightValue = element.properties.get('gableRight');
    dwu650GableArrays(dwu650GableLeftValue, dwu650GableRightValue);
    ...
    if (true == dwu650GableLeftValue) {
	    lineStyleB = { stroke: '#000', strokeWidth: '1px' };
    }
    
    if (true == dwu650GableRightValue) {
  ...
  render3D: function (element, layer, scene) {
    ...
    if (cachedJSONDoubleWallUnit650) {
      let loader = new ObjectLoader();
      let object = loader.parse(cachedJSONDoubleWallUnit650);
      return Promise.resolve(onLoadItem(object));
    }

    return loadObjWithMaterial(mtl, obj, path.dirname(img1) + '/')
      .then(object => {
        cachedJSONDoubleWallUnit650 = object.toJSON();
        let loader = new ObjectLoader();
        return onLoadItem(loader.parse(cachedJSONDoubleWallUnit650))
      });
