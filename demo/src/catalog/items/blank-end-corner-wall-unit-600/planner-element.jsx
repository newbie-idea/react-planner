...
import {sceneValues, becwu600GableArrays} from '../../utils/mtl-exports';

let globalTexture;
let worktopTexture;
let becwu600GableLeftValue;
let becwu600GableRightValue;
let pelmetValue;
let corniceValue;
let hingeNumber;
let doorNumber;
...
let cachedJSONBlankEndCornerWallUnit600 = null;

export default {
  name: 'Blank End Corner Wall Unit 600',
  prototype: 'items',

  info: {
    title: 'BECWU 600',
    tag: ['blank end', 'wall'],
    description: 'Blank End Corner Wall Unit 600',
    image: require('../../images/BECWU-600.png')
  },
  ...
  render2D: function (element, layer, scene) {
    ...
    becwu600GableLeftValue = element.properties.get('gableLeft');
    becwu600GableRightValue = element.properties.get('gableRight');
    becwu600GableArrays(becwu600GableLeftValue, becwu600GableRightValue);
    ...
    if (true == becwu600GableLeftValue) {
	    lineStyleB = { stroke: '#000', strokeWidth: '1px' };
    }
    
    if (true == becwu600GableRightValue) {
  ...
  render3D: function (element, layer, scene) {
    ...
    if (cachedJSONBlankEndCornerWallUnit600) {
      let loader = new ObjectLoader();
      let object = loader.parse(cachedJSONBlankEndCornerWallUnit600);
      return Promise.resolve(onLoadItem(object));
    }

    return loadObjWithMaterial(mtl, obj, path.dirname(img1) + '/')
      .then(object => {
        cachedJSONBlankEndCornerWallUnit600 = object.toJSON();
        let loader = new ObjectLoader();
        return onLoadItem(loader.parse(cachedJSONBlankEndCornerWallUnit600))
      });
