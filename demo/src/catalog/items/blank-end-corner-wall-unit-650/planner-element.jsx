...
import {sceneValues, becwu650GableArrays} from '../../utils/mtl-exports';

let globalTexture;
let worktopTexture;
let becwu650GableLeftValue;
let becwu650GableRightValue;
let pelmetValue;
let corniceValue;
let hingeNumber;
let doorNumber;

const mtl = require('./double-wall-unit-650.mtl');
const obj = require('./double-wall-unit-650.obj');
...
const width = {length: 650, unit: 'mm'};
const depth = {length: 300, unit: 'mm'};
const height = {length: 720, unit: 'mm'};

let cachedJSONBlankEndCornerWallUnit650 = null;

export default {
  name: 'Blank End Corner Wall Unit 650',
  prototype: 'items',

  info: {
    title: 'BECWU 650',
    tag: ['blank end', 'wall'],
    description: 'Blank End Corner Wall Unit 650',
    image: require('../../images/BECWU-650.png')
  },
  ...
  render2D: function (element, layer, scene) {
    ...
    becwu650GableLeftValue = element.properties.get('gableLeft');
    becwu650GableRightValue = element.properties.get('gableRight');
    becwu650GableArrays(becwu650GableLeftValue, becwu650GableRightValue);
    ...
    if (true == becwu650GableLeftValue) {
	    lineStyleB = { stroke: '#000', strokeWidth: '1px' };
    }
    
    if (true == becwu650GableRightValue) {
  ...
  render3D: function (element, layer, scene) {
    ...
    if (cachedJSONBlankEndCornerWallUnit650) {
      let loader = new ObjectLoader();
      let object = loader.parse(cachedJSONBlankEndCornerWallUnit650);
      return Promise.resolve(onLoadItem(object));
    }

    return loadObjWithMaterial(mtl, obj, path.dirname(img1) + '/')
      .then(object => {
        cachedJSONBlankEndCornerWallUnit650 = object.toJSON();
        let loader = new ObjectLoader();
        return onLoadItem(loader.parse(cachedJSONBlankEndCornerWallUnit650))
      });
