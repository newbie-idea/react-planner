...
import {sceneValues, dwu750GableArrays} from '../../utils/mtl-exports';

let globalTexture;
let worktopTexture;
let dwu750GableLeftValue;
let dwu750GableRightValue;
let pelmetValue;
let corniceValue;
let hingeNumber;
...
let cachedJSONDoubleWallUnit750 = null;

export default {
  name: 'Double Wall Unit 750',
  prototype: 'items',

  info: {
    title: 'DWU 750',
    tag: ['double', 'wall'],
    description: 'Double Wall Unit 750',
    image: require('../../images/DWU-750.png')
  },
  ...
  render2D: function (element, layer, scene) {
    ...
    dwu750GableLeftValue = element.properties.get('gableLeft');
    dwu750GableRightValue = element.properties.get('gableRight');
    dwu750GableArrays(dwu750GableLeftValue, dwu750GableRightValue);
    ...
    if (true == dwu750GableLeftValue) {
	    lineStyleB = { stroke: '#000', strokeWidth: '1px' };
    }
    
    if (true == dwu750GableRightValue) {
  ...
  render3D: function (element, layer, scene) {
    ...
    if (cachedJSONDoubleWallUnit750) {
      let loader = new ObjectLoader();
      let object = loader.parse(cachedJSONDoubleWallUnit750);
      return Promise.resolve(onLoadItem(object));
    }

    return loadObjWithMaterial(mtl, obj, path.dirname(img1) + '/')
      .then(object => {
        cachedJSONDoubleWallUnit750 = object.toJSON();
        let loader = new ObjectLoader();
        return onLoadItem(loader.parse(cachedJSONDoubleWallUnit750))
      });
