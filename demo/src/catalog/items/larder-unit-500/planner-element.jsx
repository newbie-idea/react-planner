...
import {sceneValues, lu500GableArrays} from '../../utils/mtl-exports';

let globalTexture;
let worktopTexture;
let lu500GableLeftValue;
let lu500GableRightValue;
let kickboardValue;
let hingeNumber;

const mtl = require('./larder-unit-500.mtl');
const obj = require('./larder-unit-500.obj');
...
const width = {length: 500, unit: 'mm'};
const depth = {length: 590, unit: 'mm'};
const height = {length: 2120, unit: 'mm'};

let cachedJSONLarderUnit500 = null;

export default {
  name: 'Larder Unit 500',
  prototype: 'items',

  info: {
    title: 'LU 500',
    tag: ['single', 'base'],
    description: 'Larder Unit 500',
    image: require('../../images/LU-500.png')
  },
  ...
  render2D: function (element, layer, scene) {
    ...
    lu500GableLeftValue = element.properties.get('gableLeft');
    lu500GableRightValue = element.properties.get('gableRight');
    lu500GableArrays(lu500GableLeftValue, lu500GableRightValue);
    ...
    if (true == lu500GableLeftValue) {
	    lineStyleB = { stroke: '#000', strokeWidth: '1px' };
    }
    
    if (true == lu500GableRightValue) {
  ...
  render3D: function (element, layer, scene) {
    ...
    if (cachedJSONLarderUnit500) {
      let loader = new ObjectLoader();
      let object = loader.parse(cachedJSONLarderUnit500);
      return Promise.resolve(onLoadItem(object));
    }

    return loadObjWithMaterial(mtl, obj, path.dirname(img1) + '/')
      .then(object => {
        cachedJSONLarderUnit500 = object.toJSON();
        let loader = new ObjectLoader();
        return onLoadItem(loader.parse(cachedJSONLarderUnit500))
      });
