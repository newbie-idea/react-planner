...
import {sceneValues, becwu900GableArrays} from '../../utils/mtl-exports';

let globalTexture;
let worktopTexture;
let becwu900GableLeftValue;
let becwu900GableRightValue;
let pelmetValue;
let corniceValue;
let hingeNumber;
let doorNumber;

const mtl = require('./double-wall-unit-900.mtl');
const obj = require('./double-wall-unit-900.obj');
...
const width = {length: 900, unit: 'mm'};
const depth = {length: 300, unit: 'mm'};
const height = {length: 720, unit: 'mm'};

let cachedJSONBlankEndCornerWallUnit900 = null;

export default {
  name: 'Blank End Corner Wall Unit 900',
  prototype: 'items',

  info: {
    title: 'BECWU 900',
    tag: ['blank end', 'wall'],
    description: 'Blank End Corner Wall Unit 900',
    image: require('../../images/BECWU-900.png')
  },
  ...
  render2D: function (element, layer, scene) {
    ...
    becwu900GableLeftValue = element.properties.get('gableLeft');
    becwu900GableRightValue = element.properties.get('gableRight');
    becwu900GableArrays(becwu900GableLeftValue, becwu900GableRightValue);
    ...
    if (true == becwu900GableLeftValue) {
	    lineStyleB = { stroke: '#000', strokeWidth: '1px' };
    }
    
    if (true == becwu900GableRightValue) {
  ...
  render3D: function (element, layer, scene) {
    ...
    if (cachedJSONBlankEndCornerWallUnit900) {
      let loader = new ObjectLoader();
      let object = loader.parse(cachedJSONBlankEndCornerWallUnit900);
      return Promise.resolve(onLoadItem(object));
    }

    return loadObjWithMaterial(mtl, obj, path.dirname(img1) + '/')
      .then(object => {
        cachedJSONBlankEndCornerWallUnit900 = object.toJSON();
        let loader = new ObjectLoader();
        return onLoadItem(loader.parse(cachedJSONBlankEndCornerWallUnit900))
      });
