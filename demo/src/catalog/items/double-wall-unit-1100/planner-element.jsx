...
import {sceneValues, dwu1100GableArrays} from '../../utils/mtl-exports';

let globalTexture;
let worktopTexture;
let dwu1100GableLeftValue;
let dwu1100GableRightValue;
let pelmetValue;
let corniceValue;
let hingeNumber;

const mtl = require('./double-wall-unit-1100.mtl');
const obj = require('./double-wall-unit-1100.obj');
...
let cachedJSONDoubleWallUnit1100 = null;

export default {
  name: 'Double Wall Unit 1100',
  prototype: 'items',

  info: {
    title: 'DWU 1100',
    tag: ['double', 'wall'],
    description: 'Double Wall Unit 1100',
    image: require('../../images/DWU-1100.png')
  },
  ...
  render2D: function (element, layer, scene) {
    ...
    dwu1100GableLeftValue = element.properties.get('gableLeft');
    dwu1100GableRightValue = element.properties.get('gableRight');
    dwu1100GableArrays(dwu1100GableLeftValue, dwu1100GableRightValue);
    ...
    if (true == dwu1100GableLeftValue) {
	    lineStyleB = { stroke: '#000', strokeWidth: '1px' };
    }
    
    if (true == dwu1100GableRightValue) {
  ...
  render3D: function (element, layer, scene) {
    ...
    if (cachedJSONDoubleWallUnit1100) {
      let loader = new ObjectLoader();
      let object = loader.parse(cachedJSONDoubleWallUnit1100);
      return Promise.resolve(onLoadItem(object));
    }

    return loadObjWithMaterial(mtl, obj, path.dirname(img1) + '/')
      .then(object => {
        cachedJSONDoubleWallUnit1100 = object.toJSON();
        let loader = new ObjectLoader();
        return onLoadItem(loader.parse(cachedJSONDoubleWallUnit1100))
      });
