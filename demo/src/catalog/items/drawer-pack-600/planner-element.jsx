...
import {sceneValues, dp600GableArrays} from '../../utils/mtl-exports';

let globalTexture;
let worktopTexture;
let dp600GableLeftValue;
let dp600GableRightValue;
let kickboardValue;

const mtl = require('./drawer-pack-600.mtl');
const obj = require('./drawer-pack-600.obj');
...
let cachedJSONDrawerPack600 = null;

export default {
  name: 'Drawer Pack 600',
  prototype: 'items',

  info: {
    title: 'DP 600',
    tag: ['single', 'base'],
    description: 'Drawer Pack 600',
    image: require('../../images/DP-600.png')
  },
  ...
  render2D: function (element, layer, scene) {
    ...
    dp600GableLeftValue = element.properties.get('gableLeft');
    dp600GableRightValue = element.properties.get('gableRight');
    dp600GableArrays(dp600GableLeftValue, dp600GableRightValue);
    ...
    if (true == dp600GableLeftValue) {
	    lineStyleB = { stroke: '#000', strokeWidth: '1px' };
    }
    
    if (true == dp600GableRightValue) {
  ...
  render3D: function (element, layer, scene) {
    ...
    if (cachedJSONDrawerPack600) {
      let loader = new ObjectLoader();
      let object = loader.parse(cachedJSONDrawerPack600);
      return Promise.resolve(onLoadItem(object));
    }

    return loadObjWithMaterial(mtl, obj, path.dirname(img1) + '/')
      .then(object => {
        cachedJSONDrawerPack600 = object.toJSON();
        let loader = new ObjectLoader();
        return onLoadItem(loader.parse(cachedJSONDrawerPack600))
      });
