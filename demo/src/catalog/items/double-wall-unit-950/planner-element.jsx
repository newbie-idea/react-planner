...
import {sceneValues, dwu950GableArrays} from '../../utils/mtl-exports';

let globalTexture;
let worktopTexture;
let dwu950GableLeftValue;
let dwu950GableRightValue;
let pelmetValue;
let corniceValue;
let hingeNumber;

const mtl = require('./double-wall-unit-950.mtl');
const obj = require('./double-wall-unit-950.obj');
...
let cachedJSONDoubleWallUnit950 = null;

export default {
  name: 'Double Wall Unit 950',
  prototype: 'items',

  info: {
    title: 'DWU 950',
    tag: ['double', 'wall'],
    description: 'Double Wall Unit 950',
    image: require('../../images/DWU-950.png')
  },
  ...
  render2D: function (element, layer, scene) {
    ...
    dwu950GableLeftValue = element.properties.get('gableLeft');
    dwu950GableRightValue = element.properties.get('gableRight');
    dwu950GableArrays(dwu950GableLeftValue, dwu950GableRightValue);
    ...
    if (true == dwu950GableLeftValue) {
	    lineStyleB = { stroke: '#000', strokeWidth: '1px' };
    }
    
    if (true == dwu950GableRightValue) {
  ...
  render3D: function (element, layer, scene) {
    ...
    if (cachedJSONDoubleWallUnit950) {
      let loader = new ObjectLoader();
      let object = loader.parse(cachedJSONDoubleWallUnit950);
      return Promise.resolve(onLoadItem(object));
    }

    return loadObjWithMaterial(mtl, obj, path.dirname(img1) + '/')
      .then(object => {
        cachedJSONDoubleWallUnit950 = object.toJSON();
        let loader = new ObjectLoader();
        return onLoadItem(loader.parse(cachedJSONDoubleWallUnit950))
      });
