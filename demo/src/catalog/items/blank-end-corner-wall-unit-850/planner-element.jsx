...
import {sceneValues, becwu850GableArrays} from '../../utils/mtl-exports';

let globalTexture;
let worktopTexture;
let becwu850GableLeftValue;
let becwu850GableRightValue;
let pelmetValue;
let corniceValue;
let hingeNumber;
let doorNumber;

const mtl = require('./double-wall-unit-850.mtl');
const obj = require('./double-wall-unit-850.obj');
...
const width = {length: 850, unit: 'mm'};
const depth = {length: 300, unit: 'mm'};
const height = {length: 720, unit: 'mm'};

let cachedJSONBlankEndCornerWallUnit850 = null;

export default {
  name: 'Blank End Corner Wall Unit 850',
  prototype: 'items',

  info: {
    title: 'BECWU 850',
    tag: ['blank end', 'wall'],
    description: 'Blank End Corner Wall Unit 850',
    image: require('../../images/BECWU-850.png')
  },
  ...
  render2D: function (element, layer, scene) {
    ...
    becwu850GableLeftValue = element.properties.get('gableLeft');
    becwu850GableRightValue = element.properties.get('gableRight');
    becwu850GableArrays(becwu850GableLeftValue, becwu850GableRightValue);
    ...
    if (true == becwu850GableLeftValue) {
	    lineStyleB = { stroke: '#000', strokeWidth: '1px' };
    }
    
    if (true == becwu850GableRightValue) {
  ...
  render3D: function (element, layer, scene) {
    ...
    if (cachedJSONBlankEndCornerWallUnit850) {
      let loader = new ObjectLoader();
      let object = loader.parse(cachedJSONBlankEndCornerWallUnit850);
      return Promise.resolve(onLoadItem(object));
    }

    return loadObjWithMaterial(mtl, obj, path.dirname(img1) + '/')
      .then(object => {
        cachedJSONBlankEndCornerWallUnit850 = object.toJSON();
        let loader = new ObjectLoader();
        return onLoadItem(loader.parse(cachedJSONBlankEndCornerWallUnit850))
      });
