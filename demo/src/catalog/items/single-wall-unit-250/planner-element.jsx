...
import {sceneValues, swu250GableArrays} from '../../utils/mtl-exports';

let globalTexture;
let worktopTexture;
let swu250GableLeftValue;
let swu250GableRightValue;
let pelmetValue;
let corniceValue;
let hingeNumber;

const mtl = require('./single-wall-unit-250.mtl');
const obj = require('./single-wall-unit-250.obj');
...
let cachedJSONSingleWallUnit250 = null;

export default {
  name: 'Single Wall Unit 250',
  prototype: 'items',

  info: {
    title: 'SWU 250',
    tag: ['single', 'wall'],
    description: 'Single Wall Unit 250',
    image: require('../../images/SWU-250.png')
  },
  ...
  render2D: function (element, layer, scene) {
    ...
    swu250GableLeftValue = element.properties.get('gableLeft');
    swu250GableRightValue = element.properties.get('gableRight');
    swu250GableArrays(swu250GableLeftValue, swu250GableRightValue);
    ...
    if (true == swu250GableLeftValue) {
	    lineStyleB = { stroke: '#000', strokeWidth: '1px' };
    }
    
    if (true == swu250GableRightValue) {
  ...
  render3D: function (element, layer, scene) {
    ...
    if (cachedJSONSingleWallUnit250) {
      let loader = new ObjectLoader();
      let object = loader.parse(cachedJSONSingleWallUnit250);
      return Promise.resolve(onLoadItem(object));
    }

    return loadObjWithMaterial(mtl, obj, path.dirname(img1) + '/')
      .then(object => {
        cachedJSONSingleWallUnit250 = object.toJSON();
        let loader = new ObjectLoader();
        return onLoadItem(loader.parse(cachedJSONSingleWallUnit250))
      });
