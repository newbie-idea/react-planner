...
import {sceneValues, ts100GableArrays} from '../../utils/mtl-exports';

let globalTexture;
let worktopTexture;
let ts100GableLeftValue;
let ts100GableRightValue;
let kickboardValue;
let hingeNumber;

const mtl = require('./tray-space-100.mtl');
const obj = require('./tray-space-100.obj');
...
const width = {length: 100, unit: 'mm'};
const depth = {length: 590, unit: 'mm'};
const height = {length: 870, unit: 'mm'};

let cachedJSONTraySpace100 = null;
...
export default {
  ...
  info: {
    title: 'TS 100',
    tag: ['single', 'base'],
    description: 'Tray Space 100',
    image: require('../../images/TS-100.png')
  },
  ...
  render2D: function (element, layer, scene) {
    ...
    ts100GableLeftValue = element.properties.get('gableLeft');
    ts100GableRightValue = element.properties.get('gableRight');
    ts100GableArrays(ts100GableLeftValue, ts100GableRightValue);
    ...
    if (true == ts100GableLeftValue) {
	    lineStyleB = { stroke: '#000', strokeWidth: '1px' };
    }
    
    if (true == ts100GableRightValue) {
  ...
  render3D: function (element, layer, scene) {
    ...
    if (cachedJSONTraySpace100) {
      let loader = new ObjectLoader();
      let object = loader.parse(cachedJSONTraySpace100);
      return Promise.resolve(onLoadItem(object));
    }

    return loadObjWithMaterial(mtl, obj, path.dirname(img1) + '/')
      .then(object => {
        cachedJSONTraySpace100 = object.toJSON();
        let loader = new ObjectLoader();
        return onLoadItem(loader.parse(cachedJSONTraySpace100))
      });
