...
import {sceneValues, dbu1050GableArrays} from '../../utils/mtl-exports';

let globalTexture;
let worktopTexture;
let dbu1050GableLeftValue;
let dbu1050GableRightValue;
let kickboardValue;
let hingeNumber;
...
let cachedJSONDoubleBaseUnit1050 = null;

export default {
  name: 'Double Base Unit 1050',
  prototype: 'items',

  info: {
    title: 'DBU 1050',
    tag: ['double', 'base'],
    description: 'Double Base Unit 1050',
    image: require('../../images/DBU-1050.png')
  },
  ...
  render2D: function (element, layer, scene) {
    ...
    dbu1050GableLeftValue = element.properties.get('gableLeft');
    dbu1050GableRightValue = element.properties.get('gableRight');
    dbu1050GableArrays(dbu1050GableLeftValue, dbu1050GableRightValue);
    ...
    if (true == dbu1050GableLeftValue) {
	    lineStyleB = { stroke: '#000', strokeWidth: '1px' };
    }
    
    if (true == dbu1050GableRightValue) {
  ...
  render3D: function (element, layer, scene) {
    ...
    if (cachedJSONDoubleBaseUnit1050) {
      let loader = new ObjectLoader();
      let object = loader.parse(cachedJSONDoubleBaseUnit1050);
      return Promise.resolve(onLoadItem(object));
    }

    return loadObjWithMaterial(mtl, obj, path.dirname(img1) + '/')
      .then(object => {
        cachedJSONDoubleBaseUnit1050 = object.toJSON();
        let loader = new ObjectLoader();
        return onLoadItem(loader.parse(cachedJSONDoubleBaseUnit1050))
      });
